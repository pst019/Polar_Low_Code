...
csv_dir= "mergedtracks2/"
csv_dir_out= "mergedtracks_tomerge/"
...
for track_year in np.arange(1979, 2021):
    print('track year', track_year,  f'time: {round(time.perf_counter() - start,2)} seconds')
    ...
    for index in track_ID_list:
        ...
        if np.shape(track) == (7,2): continue #in this case the track has only one time step
        ...
        # else: window= len(vo) -(len(vo)+1)%2 #to get the largest uneven number smaller than the length of vorticity
            ...
            track= track.drop(columns= 'vo_smooth')
    ...
    ts_after += len(red_tracks)

    if write:
        outfile= Mediadir+"/data/ERA5_Clim/track_lists/"+csv_dir_out+csv_name+f'_post-process_{track_year}.csv'
        print('write:', outfile)
        red_tracks.to_csv(outfile) 

        
print('\n Nr time steps prior:', ts_prior)
...
print('remain in list after splitting:', ts_after)
print('Number of splits', total_splits)


alltracks=pd.DataFrame( )

for track_year in np.arange(1979, 2021):
    tracks_year= pd.read_csv(Mediadir+"/data/ERA5_Clim/track_lists/"+csv_dir_out+csv_name+f'_post-process_{track_year}.csv')

    alltracks= alltracks.append(tracks_year)


print('be aware that boundary repeater of lifetime shorter than 6h are still in the output list')
# durlim= 6
...
if write:
    outfile= Mediadir+"/data/ERA5_Clim/track_lists/"+csv_dir+csv_name+f'_post-process.csv'
    print('write:', outfile)
    alltracks.to_csv(outfile) 
