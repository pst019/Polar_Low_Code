...
save=True
# save= False
savedir= homedir + '/Polar_Low/Paper_ERA5_PLclim/Figs/PLvsTRACKS_distr_7/'
...
tracks_after_PL_crit= True
legend= False
vertical_line= True #line where the criteria lies
...
excl_crit_list= [
                ...
                ['theta_diff_500-sst_mean250_min', 11.0, 'smaller'],
                # ['theta_diff_500-sst_mean250_min', 13.2, 'smaller'],

                # ['SST-T_500_mean250_max', 41.5, 'larger'],
                # ['SST-T_500_mean250_max', 38.4, 'larger'],

                ['theta_trop_mean250_mean', 300.8, 'smaller'],
...
# plotvar, plotchar, bins, plotname='theta_trop_mean250', 'mean' , np.arange(265, 379, 1), r'$\theta_{trop}$ [K]'
...
plotvar, plotchar, PLs_larger, bins, plotname='vo_max', False, True, np.arange(0, 89.1, 0.5), 'Relative vorticity [10$^{-5}$s$^{-1}$]'
# plotvar, plotchar, PLs_larger, plotname= 'vo_tendency', False, True , r'Vorticity tend [10^{-5}$s$^{-1}$/h]'

# plotvar, plotchar, bins, plotname='slp', False, np.arange(945, 1029.1, 1), 'Sea-level pressure [hPa]' #the lifetime min

# plotvar, plotchar='vortex_type', False
# plotvar, plotchar, bins, plotname='diameter_max', False, np.arange(0, 899, 20), 'Vortex diameter [km]'
# plotvar, plotchar, bins, PLs_larger, plotname= 'duration', False, np.arange(0, 90.1, 1), True, 'Lifetime [h]'
...
for system, system_name in zip(systems, systems_name_list):
    ...
    if time_start != False and system_name == 'tracks NH':
        ...
        system_params= system_params.rename(columns= {'land_dist':  'land_dist_old', 'land_dist_correct': 'land_dist'}) #make the correct land dist the default one
    else: #for the PLs
    ...
    if plotvar== 'slp':
        system_ind[plotvar]= system.groupby(['ID']).min()[plotvar]
...
if tracks_after_PL_crit: systems_ind = systems_ind[:2]+ [tracks_ind_crit, tracks_ind_SH_crit]+ systems_ind[2:]
if tracks_after_PL_crit: systems_name_list =systems_name_list[:2] + ['tracks NH after crit', 'tracks SH after crit'] + systems_name_list[2:]
...
if tracks_after_PL_crit == False: mpl.rcParams['axes.prop_cycle']= cycler(color= ['k', 'k', "#800000FF", "#767676FF", "#FFA319FF" ,"#8A9045FF", "#155F83FF", "#C16622FF"])
...
if vertical_line:
    if plotvar== 'theta_diff_500-sst_mean250':
        crit_list.sort()
        crit_weak= crit_list[-2]
        
    plt.axvline(crit_weak, 0, 1, c= 'r', ls= '--')
...
if save:
    ...
    if tracks_after_PL_crit == False: savefile += '_no-tracks-crit'
    print(savefile)
