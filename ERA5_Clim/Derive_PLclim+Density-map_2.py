...
Plot_take_time= True #make a plot that calculates the PLs within a given distance
...
if import_PLs:
    ...
    for vc in var_char_ind:
        ...
        if varname in ['theta_diff_500-sst_mean250', 'SST-T_500_mean250_max']:
...
if Plot_take_time:
    ...
    if plot_log== True:
        ...
        if ['theta_diff_500-sst_mean250', 'min', 11.0, 'smaller'] in var_char_ind:
            bounds= [0, .1, .2, .5, 1, 2, 3, 4, 6, 8]
        if ['vo', 'max', 30, 'larger'] in var_char_ind:
            bounds= [0, .1, .2, .5, 1, 2, 3, 4]
        if ['diameter', 'max', 350, 'smaller'] in var_char_ind:
            bounds= [0, .1, .2, .5, 1, 2, 3, 4, 5]
            
        if ['SST-T_500_mean250', 'max', 44, 'larger'] in var_char_ind:
            bounds= [0, .1, .2, .5, 1, 2, 3, 4]
    ...
    cb.set_label(f'Annual time with a polar-low centre within {per_dist}km distance [d]', size=14)
