...
for index in track_ID_list:
    ...
    # else: window= len(vo) -(len(vo)+1)%2 #to get the largest uneven number smaller than the length of vorticity
        ...
        local_max_0= signal.argrelextrema(track_now['vo_smooth'].where(track_now['vo_smooth'] > threshold_vo_max_min).values, np.greater)[0]
        ...
        if len(local_max_0) >= 2:
            ...
            local_min= signal.argrelextrema(track_now['vo_smooth'].values, np.less)[0]
            ...
            if Plot:
                ...
                plt.plot(track_now.iloc[local_max_0]['time'], track_now.iloc[local_max_0]['vo_smooth'], 'o', color='orange')
                plt.plot(track_now.iloc[local_min]['time'], track_now.iloc[local_min]['vo_smooth'], 'v', color= 'orange')
            ...
            #to exclude "Wendepunkte" - small local max and min
                ...
                if len(local_max) >= 2:
                    ...
                    if Plot:
                        plt.plot(track_now.iloc[local_max]['time'], track_now.iloc[local_max]['vo_smooth'], 'o', color='r')
                        plt.plot(track_now.iloc[local_min]['time'], track_now.iloc[local_min]['vo_smooth'], 'v', color= 'r')
