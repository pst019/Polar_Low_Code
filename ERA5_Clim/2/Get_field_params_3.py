...
"""get all tracks"""
if to_list=='tracks':
    track_dir= Mediadir+'/data/ERA5_Clim/track_lists/mergedtracks/merged_tracks_'+version+'_'+ending+f'_{start_year}-{end_year}'
    
    if durlim: track_dir += 'durlim'+str(durlim)
    tracks= pd.read_csv(track_dir+'.csv')

else:
    """get matchPL list"""
    track_dir= Mediadir+f'/data/ERA5_Clim/track_lists/matchPLlist/matchPLlist_{to_list}_dist150'
    tracks= pd.read_csv(track_dir+'.csv')
...
for time_step in time_steps:
    ...
    if param_var== 'shear_925-500':
        ...
        # ds_z= xr.open_dataset(Mediadir + f"data/ERA5_Clim/ERA5_data/{file_name_z}_era5_{time_step.year}_{time_step.month:02}.nc")
    ...
    for i in range(len(tracks_now)): 
        ...
        if param_var == 'shear_925-500':
            u_0= value_in_rad(ds['u'].isel(plev= 0), ds.lat, ds.lon, track.lat, track.lon, distance= param_rad)
            v_0= value_in_rad(ds['v'].isel(plev= 0), ds.lat, ds.lon, track.lat, track.lon, distance= param_rad)
            u_1= value_in_rad(ds['u'].isel(plev= 1), ds.lat, ds.lon, track.lat, track.lon, distance= param_rad)
            v_1= value_in_rad(ds['v'].isel(plev= 1), ds.lat, ds.lon, track.lat, track.lon, distance= param_rad)

            # z_0= value_in_rad(ds_z['z'].isel(plev= 0), ds_z.lat, ds_z.lon, track.lat, track.lon, distance= param_rad)
            # z_1= value_in_rad(ds_z['z'].isel(plev= 1), ds_z.lat, ds_z.lon, track.lat, track.lon, distance= param_rad)
            
            du= u_1- u_0
            dv= v_1- v_0
       
            shear_angle= UV2Direction(du, dv) #in compass direction
            
            dU= np.sqrt(du**2 + dv**2)
            dH= 5000 # (z_1 - z_0 )/9.81
