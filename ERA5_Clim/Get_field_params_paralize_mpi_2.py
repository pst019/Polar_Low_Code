...
Run via mpirun -n 32 python Get_field_params_paralize_mpi_2.py
"""
...
set_name= 'Rojo'
#set_name= 'Golubkin'
set_name='tracks' #to which list parameters should be derived
...
#param_var, param_type= 'theta_trop', 'mean' #1year - 90s
#param_var, param_type= 'U_trop', 'max'
# param_var= 'U_trop_polew' #not calculated with param_type
#param_var, param_type='U_surface', 'max' #1y - 300s
param_rad= 250
...
param= param_var
if param_type: param += '_'+param_type+ str(param_rad)


if param_var == 'shear_925-500':
    param1= param[:5]+'_angle'+param[5:]
    param2= param[:5]+'_strength'+param[5:]
    param_var_list= [param1, param2]

# time_start= False
time_start= '2008-1-1' #the field data is needed for this time
time_end= '2008-1-3'

# time_start= '1980-1-1' #the field data is needed for this time
# time_end= '2019-12-31'
...
def get_track_for_list(set_name, time_start, time_end, version= version, ending= ending,
    ...
    if set_name=='tracks':
        ...
        """get matchPL list"""
        track_dir= Mediadir+f"/matchPLlist/matchPLlist_{set_name}_dist{matchdist}"
        if time_diff and set_name != 'Yanase': track_dir += f"_timediff{time_diff}"
...
def calc_param_to_list(time_steps, tracks, param= param, 
                       param_var= param_var, param_type= param_type, workdir= workdir):
    param_list= dict()
    ...
    for time_step in time_steps:
        ...
        if param_var== 'theta_diff_500-925':
            ...
            if fram:
                ...
                theta_0= PotTemp(ds['var130'].isel(plev=0), ds.plev[0]/100),
            else:
        ...
        if param_var== 'N_500-925': #not tested
            ...
            h_diff=  (ds2['var129'].isel(plev=1) - ds2['var129'].isel(plev=0) )/9.81
        ...
        if param_var== 'theta_diff_500-skt':
            ...
            if fram: theta_1= PotTemp(ds['var130'].isel(plev=1), ds.plev[1]/100)
            ...
            if fram: theta_0= PotTemp(ds2['var235'], ds3['var151']/100)
        ...
        for i in range(len(tracks_now)): 
            ...
            #     tracks.loc[track.name, param]= float(ds[param_var].where((
            #         np.abs(ds.lon-track.lon) < 0.5) & (ds.lat >= track.lat)).max())
            ...
            if param_var == 'shear_925-500':
                ...
                param_list[track.name]=  [shear_angle, shear_strength]


            elif param_var== 'land_dist':
                dist= (110* np.sqrt( (ds.lat - track.lat)**2 + (np.cos(np.deg2rad(track.lat)) *(ds.lon- track.lon) )**2 ) ).values
    
                # tracks.loc[track.name, param]= np.min(dist[ds.lsm.values > 0])   
                param_list[track.name]= np.min(dist[ds['var172'].values > 0])   #exclude distances over water
...
tracks, track_dir= get_track_for_list(set_name, time_start, time_end)

time_steps = np.array(remove_dublicate(tracks.time))


nx_r= int(len(time_steps)/size)
if rank == 0: print('nx_r', nx_r)


time_steps_core= time_steps[int(rank*nx_r) : int((rank +1)*nx_r)]


param_list= calc_param_to_list(time_steps_core, tracks, param, param_var, param_type, workdir)

param_list = comm.gather(param_list, root=0)

if rank == 0:
    
    """merge the individual dicts from each independent run into the tracks dataframe"""
    total_param_dict= {}
    for param_list_part in param_list:
        total_param_dict.update(param_list_part)
    
    param_df= pd.DataFrame(total_param_dict.items(), columns=['Index', param])
    param_df= param_df.set_index(pd.MultiIndex.from_tuples(param_df.Index, names= ['ID', 'step'])).drop(columns='Index')

    if param_var == 'shear_925-500':
        param_df= pd.DataFrame(param_df[param].to_list(), columns=param_var_list, index= param_df.index)
        param= param_var_list

    tracks= tracks.merge(param_df, left_index=True, right_index=True)
