...
# Contour labels
plt.clabel(cmslp, fontsize=9, inline=1) 
...
cwind = m.contourf(x,y,U10_2[t,-1,:,:],cmap=plt.cm.YlGnBu)
cb = m.colorbar(cwind,"right", size="5%", pad="2%")
cb.set_label('Wind speed 10 m (m/s)')
...
#m = make_map(ncfile=nc2)
#x,y = m(lon,lat)
#cwind = m.contourf(x,y,U10_2[t,-1,:,:]-U10[t,-1,:,:],cmap=plt.cm.YlGnBu)
#cb = m.colorbar(cwind,"right", size="5%", pad="2%")
#cb.set_label('Wind speed 10 m (m/s)')
#
#plt.subplot(2, 2, 4)
#m = make_map(ncfile=nc2)
#x,y = m(lon,lat)
#cb = m.contourf(x,y,(mslp_2[t,0,:,:]-mslp[t,0,:,:])/100.,cmap=plt.cm.YlGnBu)
