...
mainfig= plt.figure(fignr, figsize= (7,11))
fignr+=1
plt.clf()
...
#legendlabel= ['CTR', 'noQH', 'noCondens', 'noTH', 'noFLX', 'noFLXarea', '2FLX']
...
#        cpt= [np.where(d.lon== windmaxlon[t]) for t in tcycl]
        ...
        if i < maxnri:
            ...
            ax5.plot(d.datetime[tcycl_red], SenHMean, color= color, linestyle=marker[i])#, label= str(cyclnr))
            ax5.plot(d.datetime[tcycl_red], LatHMean, color= color, linestyle=marker[i+1])#, label= str(cyclnr))
            ax5.plot(d.datetime[tcycl_red], CondensMean, color= color, linestyle=marker[i+2])#, label= str(cyclnr))
...
ax5.set_ylabel("Heat fluxes [W m$^{-2}$]", fontsize= 12)
...
ax5.legend(['Sensible flux', 'Latent flux', 'Latent release'], fontsize= 12, ncol= 1) #be careful with the order
