...
for t in tlist:
    ...
    if var in ['FLX', 'LH', 'LH_thor', 'TH', 'TH_thor', 'PBH', 'CAPE', 'CIN', 'LCL', 'LFC', 'OLW'] or 'Cloud' in var:
        d.imp_atm(tn= t)
    ...
    #    PlotContours(Lon, Lat, d.geop, map, leveldist= 10)
        if d.chosen_plevel == 500: bounds= np.arange(278, 291, 1)
...
#        bounds= np.arange(260, 291, 2)
        ...
        theta= PotTemp(d.T, d.chosen_plevel)
        bounds, colors=PlotColorMap4(Lon, Lat, theta, map, bounddist= 1, color='RdBu', label=r"$\theta_{"+str(d.chosen_plevel)+"}$ [K]")
...
#        PlotColorMap4(Lon, Lat, TFP_filter, map, color= 'RdBu', symetric=True, label=r"TFP$_{"+str(d.chosen_plevel)+"}$")
    ...
    if var == 'Geop_wind_lev':
        PlotContours(Lon, Lat, d.geop, map, leveldist= 10, alpha= .7)
    #    PlotWindVelo(Lon, Lat, np.sqrt(d.u**2+ d.v**2), map)
        ...
        PlotWind2(d.lon, d.lat, d.u, d.v, map, rot=False, everyx= 13, everyy= 13, color='white')
...
#        wbounds= [-5, -1, -0.5, -0.2, -0.1, 0.1, 0.2, 0.5, 1, 5]
    ...
    if var== 'SH_lev':
        PlotContours(Lon, Lat, d.mslp, map, leveldist= pleveldist)
        bounds, colors= PlotColorMap4(Lon, Lat, d.SH, map, symetric=False, label='Specific humidity [kg/kg] at ' +str(d.chosen_plevel)+'hPa', color='blue', bounds= np.arange(0, np.max(d.SH)*1E4, 1)*1E-4)
