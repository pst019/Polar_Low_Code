...
mainfig= plt.figure(fignr, figsize= (7,10))
fignr+=1
plt.clf()
...
#exp_name, label_name= 'DA_080303_p6SST', 'p6SST'
...
#    if var== 'Baroclin_lev':
    d.imp_uv_ft(pn= pn, additional=['T', 'SH'])
#    elif var== 'Baroclin_dry_lev':
#        d.imp_uv_ft(pn= pn, additional=['T'])
#    else:
#        d.imp_uv_ft(pn= pn)
    ...
    else: #if baroclinicty should be displayed
        PlotContours(Lon, Lat, theta_e_filter[t], map, leveldist= 2)
        PlotColorMap4(Lon, Lat, abs_grad_theta_e_filter[t], map, color= 'red', bounds= bounds, label= label)
...
#            if label== 'cold_pseudo2': label= 'cold'
        ...
        tlim= 29
        grad_thetacycl, grad_thetamaxlon, grad_thetamaxlat= Track_OtherMax(abs_grad_theta_e_filter[:, edgedist: -edgedist, edgedist: -edgedist], grad_thetadist, tcycl[tcycl<=tlim], latcycl[tcycl<=tlim], d.lat[edgedist: -edgedist, edgedist: -edgedist], d.lon[edgedist: -edgedist, edgedist: -edgedist], local='max')
        
#        grad_thetacycl[tcycl > 30]= np.nan
...
#            map.plot(xpt,ypt, 'o', color= 'orange')
            ...
            if var in ['Baroclin_lev', 'Baroclin_dry_lev']:
                xpt, ypt= map(grad_thetamaxlon[ind], grad_thetamaxlat[ind])
                map.plot(xpt,ypt, 'o', color= 'green')
                plt.text(xpt- 0.04*(map.xmax-map.xmin), ypt- 0.01*(map.ymax-map.ymin), str(np.round(np.round(grad_thetacycl[ind])[0], 2)), color='green', fontsize=15, fontweight= 'bold')
...
if var== 'Baroclin_dry_lev':
    ax4.set_ylabel(r"$\nabla \theta_{"+str(d.chosen_plevel)+"}$ [K/100km]", fontsize= 12)
elif var== 'Baroclin_lev':
...
if save==True:
    ...
    if var=='Baroclin_dry_lev': savename += '_Baroclin_dry'
