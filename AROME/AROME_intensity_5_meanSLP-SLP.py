...
#        slpcycl, slpmaxlon, slpmaxlat= Track_OtherMax(d.mslp_ft, slpdist, tcycl, latcycl, d.lat, d.lon, local='min')
        ...
        """create the meanSLP-SLP field and calculate local min"""
        prec_red= 4 #factor of precision rediction to make it faster
        pixrad= slpmeanrad/(2.5*AAres*prec_red) 
        local_mean= np.array([CircularFilter_samedist(d.mslp_ft[t][::prec_red, ::prec_red], radius= pixrad) for t in range(len(d.mslp_ft))])
        diffslp_field= d.mslp_ft[:, ::prec_red, ::prec_red] - local_mean

        slpdist=300

        slpcycl, slpmaxlon, slpmaxlat= Track_OtherMax(diffslp_field, slpdist, tcycl, latcycl, d.lat[::prec_red, ::prec_red], d.lon[::prec_red, ::prec_red], local='min',diff_grid_data=True, loncycl= loncycl)
...
#ax3.set_ylabel('Sea level pressure - mean(SLP) [hPa]', fontsize= 12)
ax3.set_ylabel('SLP - mean(SLP) [hPa]', fontsize= 12)
if Baro== 1: ax4.set_ylabel(Baro_label, fontsize= 12 ) #r"$\nabla \theta_{e,"+str(d.chosen_plevel)+"}$ [K/100km]", fontsize= 12)
if Heat== 1: ax5.set_ylabel("Heat fluxes [W m$^{-2}$]", fontsize= 12)
