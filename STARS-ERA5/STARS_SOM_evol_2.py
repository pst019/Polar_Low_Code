...
if 'z_ano' in cont_full: cont_levels = np.arange(-200, 201, 25)
...
if plot_cont2:
    cont_full2= cont2
    if ano_cont2==True:  cont_full2 += '_ano'

    if type(plevel_cont2) == int: cont_full2 += '_'+str(plevel_cont2)
    
    if 'z_ano' in cont_full2: cont_levels2 = np.arange(-200, 201, 25)
...
if plot_cont3:
    cont_full3= cont3
    if type(plevel_cont3) == int: cont_full3 += '_'+str(plevel_cont3)
    
    if cont3 == 'mcc': cont_levels3= [0.7, 0.8, 0.9]
...
system_char= 'mean'
#Stoll, Stoll_ind= Stoll_individual_systems(Stoll, ID_name='ID', system_char=system_char, update_S= False)
...
if load_ds:
    ...
    ds= imp_ds(var, plevel_var, var_full)

    """file for cont variable"""   
    ds2= imp_ds(cont, plevel_cont, cont_full)
#    if plevel_cont != plevel_var: ds2= ds2.rename({'plev': 'plev1'})
    
    ds= xr.merge([ds, ds2])

    if plot_cont2:
        ds2= imp_ds(cont2, plevel_cont2, cont_full2,
                    compute= compute_cont2, imp_var= imp_cont2)#, data_gausfilter= data_gausfilter3)

        ds= xr.merge([ds, ds2])

    if plot_cont3:
        ds2= imp_ds(cont3, plevel_cont3, cont_full3,
    ...
    if ano_cont2 == True:
        ds[cont_full2]= (('time', 'x', 'y'), ds[cont_full2].values- np.mean(ds[cont_full2].values, axis= (1,2))[:, np.newaxis, np.newaxis])
...
for iSOM in range(1, x*y +1):
    ...
    if plot_cont2:
        cs2= ax.contour(ds_node.x, ds_node.y, np.mean(ds_node[cont_full2], axis= 0), levels= cont_levels2, linewidths= 2, colors= 'green', linestyles='dashed')

    if plot_cont3:
        cs3= ax.contour(ds_node.x, ds_node.y, np.mean(ds_node[cont_full3], axis= 0), levels= cont_levels3, linewidths= 2, colors= 'gray')


    ax.clabel(cs, cont_levels[::2], fontsize=10, fmt='%1.0f')#, inline=1)
    if plot_cont2:
        ax.clabel(cs2, cont_levels2[::2], fontsize=10, fmt='%1.0f')#, inline=1)
    ...
    ax.scatter(0,0, marker= 'x', color= 'k', zorder= 2)
...
#        Stoll[Stoll.Obs == Stoll_ind.loc[ind].Obs_mature].loc[ind].node.values[0] #find the node of the mature stage for this PL

        if np.isnan(mature_node):
            if m1_rem: 
                S_dropna= S.dropna(subset=['node'])
                mature_node= S_dropna.iloc[S_dropna['vo'].values.argmax()].node
            else: mature_node = -1
...
if save:
    save_name=savedir+ 'SOM_evolution--mean_'+var_full+"_"+cont_full
    
    if plot_cont2: save_name+= "_"+cont_full2
    if plot_cont3: save_name+= "_"+cont_full3

#        if vo_tend_excl: save_name += '_vo-tend-'+str(vo_tend_thresh)
    save_name += "_x"+str(x)+"_y"+str(y)
    save_name += save_extra    
