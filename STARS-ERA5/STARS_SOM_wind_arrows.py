...
save= False
#save= True
savedir= homedir+ 'Polar_Low/STARS-ana/Figs/SOM6/'
#
fignr= 24
...
elif var_full== 'msl': levels= np.arange(960, 1031, 2)
...
#
    ...
    """rotate the wind with propagation direction"""
    U= np.sqrt(ds[uvar]**2 + ds[vvar]**2)
    wind_beering= UV2Direction(ds[uvar], ds[vvar])
    track_beering = ds.beering.values
    
    rot_beering= (track_beering[:, np.newaxis, np.newaxis]- wind_beering)%360
    v_r, u_r = WindSpeedDirection2UV(U, rot_beering, orientation='') #maybe u_zonal and v_meridional are exchanged in WindSpeedDirection2UV, but this seem to work"""

    ds[uvar]= (('time', 'x', 'y'), u_r )
    ds[vvar]= (('time', 'x', 'y'), v_r )

    ds[uvar].attrs['units'] = 'm/s'
    ds[uvar].attrs['long_name']='x wind'

    ds[vvar].attrs['units'] = 'm/s'
    ds[vvar].attrs['long_name']='y wind'
...
for iSOM in range(1, x*y +1):
    ...
    if levels_different:
        ...
        if only_contours== True:
            cs= plt.contour(ds_node.x, ds_node.y, np.nanmean(ds_node[var_full], axis= 0), levels= levels, colors='k')
    ...
    nth= 4
#    plt.quiver(ds_node.x[::nth], ds_node.y[::nth], np.mean(ds_node[uvar], axis= 0)[::nth,::nth], np.mean(ds_node[vvar], axis= 0)[::nth,::nth], pivot='mid', width= 0.01)
    plt.barbs(ds_node.x[::nth], ds_node.y[::nth], np.mean(ds_node[uvar], axis= 0)[::nth,::nth], np.mean(ds_node[vvar], axis= 0)[::nth,::nth], pivot='middle', length= 4, barbcolor= 'g')
    ...
    plt.title('SOM '+str(iSOM)+ ' ('+str(np.round(perc_of_SOM*100, 1))+'%, '+ str(int(np.round(after_tend_excl/before_tend_excl *100))) + '%)')
