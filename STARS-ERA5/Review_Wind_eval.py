...
Stoll= pd.concat([Stoll, S_ERA5, S_ERA5_2], axis=1)
...
remove_dublicate(Stoll['U_10min_knots'].dropna())


Stoll.loc[Stoll['U_10min_knots'] == '#35','U_10min_knots'] = '37.5'

Stoll['U_10min_knots'] = Stoll['U_10min_knots'].astype(float)

StollnoNA= Stoll.loc[:, ['U_10min_knots', var] ].dropna()

plt.plot(StollnoNA['U_10min_knots']/2, StollnoNA[var], 'x')
      
plt.plot(np.arange(10, 35), np.arange(10,35), 'k')


z = np.polyfit(StollnoNA['U_10min_knots']/2, StollnoNA[var], 1)
p = np.poly1d(z)
plt.plot(np.arange(10, 35), p(np.arange(10,35) ),"b--")

print(np.corrcoef(StollnoNA['U_10min_knots']/2, StollnoNA[var] )[1,0])

plt.xlabel('Observed wind speed [m/s]')
plt.ylabel('ERA-5 wind speed [m/s]')
