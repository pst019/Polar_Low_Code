...
S_ERA5_3= pd.read_csv(imp_dir + 'Stoll_ERA5_3.csv')
S_ERA5_3= S_ERA5_3.set_index(['ID', 'time'])

Stoll= pd.concat([Stoll, S_shear_cats, S_ERA5, S_ERA5_2, S_ERA5_3], axis=1)
...
if var== 'vo_tendency': Stoll[var] *= 1E5

if vo_tend_excl== True:
    Stoll= Stoll[Stoll['vo_tendency'] > 0]
...
for node in nodelist:
    kernel= stats.gaussian_kde(Stoll[Stoll.shear_category.isin(node)][var].dropna())
...
Stoll= Stoll.dropna()
Stoll.boxplot(column = var, by = 'shear_category', ax= ax, grid=False,
...
print(Stoll.groupby(['shear_category']).median()[var]) #print the median in each node

#plt.xticks(np.arange(1,10).astype(int))

ax.set_xticklabels(['Forw.', 'Right', 'Rev.', 'Left', 'Weak'])
...
elif 'vo_tendency' in var: varlabel= r"Vort. tend. [10$^{-5}$ s$^{-1}$ h$^{-1}$]"
...
elif 'cape' in var: varlabel= 'CAPE [J kg$^{-1}$]'
