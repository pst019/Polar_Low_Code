...
print('var', var)
if var in ['q', 'z', 'vo', 'U']: print('Be careful with the units, maybe you want to adapt this')
...
#        ax.scatter(Sm_lon[Obs-1], Sm_lat[Obs-1], transform=ccrs.PlateCarree(), s= 50, marker= "s", color= 'k', zorder=2)
        ...
        if S_now.row_idx == 0:
            beering= gcc.bearing_at_p1( (Sm_lon[S_now.row_idx], Sm_lat[S_now.row_idx]), (Sm_lon[S_now.row_idx +1], Sm_lat[S_now.row_idx +1]) )
        elif S_now.row_idx == len(S_track) -1:
            beering= gcc.bearing_at_p2( (Sm_lon[S_now.row_idx-1], Sm_lat[S_now.row_idx-1]), (Sm_lon[S_now.row_idx], Sm_lat[S_now.row_idx]) )
        else:
            beering= gcc.bearing_at_p2( (Sm_lon[S_now.row_idx-1], Sm_lat[S_now.row_idx-1]), (Sm_lon[S_now.row_idx+1], Sm_lat[S_now.row_idx+1]) )
        ...
        if var=='z': ds0[var]/=9.81 # I should correct for the units
        ...
        if type(plevel) == int or plevel == '':
            if var =='lat':
                var_interp0= griddata(tuple([np.ravel(longrid), np.ravel(latgrid)]) , np.ravel(latgrid), (center_grid[:,:,0], center_grid[:,:,1]), method='linear')
...
#    cdftime = nc.netcdftime.utime(timeunits,calendar=calendar)
    ...
    if type(plevel) == int:
        ncds = nc.Dataset(Mediadir + "ERA5_STARS/PL_centred_fields_smooth-tracks/"
                      +var + '_'+ str(plevel) +'_allObs'+interp+ '_track-smth-'+ string_smooth_param +'.nc','w',format='NETCDF3_CLASSIC')
    elif plevel == '':#if problems with plevel= int this could be "else"
        ncds = nc.Dataset(Mediadir + "ERA5_STARS/PL_centred_fields_smooth-tracks/"
                      +var +'_allObs'+interp+ '_track-smth-'+ string_smooth_param +'.nc','w',format='NETCDF3_CLASSIC')
    else:#for all levels
        ncds = nc.Dataset(Mediadir + "ERA5_STARS/PL_centred_fields_smooth-tracks/"
                      +var + '_'+ plevel +'_allObs'+interp+ '_track-smth-'+ string_smooth_param +'.nc','w',format='NETCDF3_CLASSIC')            
...
#    steer_vel[:]= steer_vel_list
    ...
    beer_nc = ncds.createVariable('beering', 'f' ,('time',)) #some PLs have to be excluded.
    beer_nc.setncattr('name','Angle of the smoothed track (from North)')
    beer_nc[:]= beering_list
...
#    ncPLnr[:]= nc.stringtochar(np.array(PLnr_list, 'S'))
    ...
    if var != 'U':
        if var in ['q', 'z', 'vo', 'U']: print('Be careful with the units')
