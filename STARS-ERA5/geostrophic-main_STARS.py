...
#tdelta= np.abs((S['time'] - datetime_now) / np.timedelta64(1, 'h'))
...
#S_now['dt_round']= [dt.replace(hour= 0, minute= 0) for dt in S_now['time']]
...
#
#
#f= np.sin(np.deg2rad(S_now.lat.values[0])) * 2*np.pi/(24*60**2)
#
#
...
##plt.clf()
##
##ax= Plot_Polar_Stereo(fig, central_lon= 30, extent= [-10, 50, 60, 80], subplot= (1,1,1))
#
