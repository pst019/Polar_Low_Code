...
if typ == '':
    ...
    Stoll.loc[Stoll[speed_var] <= 1.5, 'shear_category'] = 'weak'
...
for iSOM in range(1, xSOM*ySOM +1):
    ...
    plt.scatter(x,y, zorder= 0, s= .2, c=color)
...
#    lev= 0.05 #define a specific level
    lev= np.percentile(zi, 95) #this makes 5% of the points with highest rate 
    #- maybe better to find a level such that 70% of points are included


#    print(sum(zi[zi> lev])/sum(zi)) #this counts from the density estimate how much is included in the contour
    zi= zi.reshape(xi.shape)
    plt.contour(xi, yi, zi, levels=[lev], colors= color)
#    plt.contour(xi, yi, zi.reshape(xi.shape)) #, levels=[lev], colors= color)#,  label=str(iSOM))

#    ctf = ax.contourf(S_SOM[dir_var], S_SOM[speed_var], z)
#    plt.colorbar(ctf)
#    plt.polar(np.deg2rad(S_SOM[dir_var]), S_SOM[speed_var], c=z, 'x', label=str(iSOM))   
    plt.plot(np.median(x), np.median(y), 'o', color= color,  label=str(iSOM))


    """a more precise method to calculate how many points are included by the contour"""
    totalcount= len(x)
    count= 0
    for point in range(totalcount):
        dist= (x[point]- xi)**2 + (y[point]- yi)**2
    ...
    print('points included in contour:', count/totalcount)
...
circlex = np.cos(theta)
circley = np.sin(theta)
plt.plot(1.5*circlex, 1.5*circley, c= 'k')
#plt.plot(2*circlex, 2*circley, c= 'k')

line_x= np.linspace(np.sqrt(0.5)*1.5, lim, 2)
...
if save:
    save_name=savedir+ 'classification-'+label_shear
