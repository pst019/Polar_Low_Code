...
# the almost original
#remove_list= ['-']
#S_ind['Morphology']= [" ".join(remove_from_list(remove_dublicate(S[S['ID'] == ID]['Morphology']), remove_list)) for ID in S_ind['ID']]
...
#S_ind['Morphology']= [" ".join(remove_repeater(split_items(remove_repeater(remove_from_list(S[S['ID'] == ID]['Morph_red'], remove_list)), ' - '))) for ID in S_ind['ID']]
...
morphs= S_ind.groupby('Morphology').size()
...
#    botn,b_dn,p_dn= plt.hist(S_ind[S_ind['Morphology'] == morph]['Month'], bins= np.arange(.5, 12.6, 1), label=morph, bottom= bot)
#    bot += botn
#
#plt.xlim(.5, 12.5)
#plt.xlabel('Month')
...
for morph in tab.columns:
    botn,b_dn,p_dn= plt.hist(S_ind[S_ind['Morphology'] == morph]['Duration'], bins= np.arange(0, 116, bindist), label=morph, bottom= bot)
    bot += botn
...
bot=False
for morph in tab.columns:
    botn,b_dn,p_dn= plt.hist(S_ind[S_ind['Morphology'] == morph]['Distance'], bins= np.arange(0, 2100, 50), label=morph, bottom= bot)
    bot += botn
...
bot=False
for morph in tab.columns:
    botn,b_dn,p_dn= plt.hist(S_ind[S_ind['Morphology'] == morph]['Prop_speed'], bins= np.arange(0, 100, 4), label=morph, bottom= bot)
    bot += botn
