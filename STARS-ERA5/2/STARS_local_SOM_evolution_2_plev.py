...
var_evol_p= var_evol+ '_'+str(plevel_var)

if ano_evol==True:
    var_evol_full= var_evol+ '_ano'
    var_evol_p_full= var_evol_p+ '_ano'
else:
    var_evol_full= var_evol
    var_evol_p_full= var_evol_p


ano_evol_cont=True
var_evol_cont='z'
plevel_cont=500
var_evol_cont_p= var_evol_cont+ '_'+str(plevel_cont)


if ano_evol_cont==True:
    var_evol_cont_full= var_evol_cont+ '_ano'
    var_evol_cont_p_full= var_evol_cont_p+ '_ano'
else:
    var_evol_cont_full= var_evol_cont
    var_evol_cont_p_full= var_evol_cont_p
...
#var_evol_cont2='z'
#plevel_cont2=500
#var_evol_cont2_p= var_evol_cont2+ '_'+str(plevel_cont2)
#
#
#if ano_evol_cont2==True:
#    var_evol_cont2_full= var_evol_cont2+ '_ano'
#    var_evol_cont2_p_full= var_evol_cont2_p+ '_ano'
#else:
#    var_evol_cont2_full= var_evol_cont2
#    var_evol_cont2_p_full= var_evol_cont2_p
#
#
ano_evol_cont3=False
#var_evol_cont3='blh'
#var_evol_cont3='cape'
#var_evol_cont3='tp'
#var_evol_cont3='lcc'
...
if ano_evol_cont3==True:
    var_evol_cont3_full= var_evol_cont3+ '_ano'
else:
    var_evol_cont3_full= var_evol_cont3
#    var_evol_cont2_p_full= var_evol_cont2_p


"""specify the levels"""
if var_evol_full == 't_ano':  levels = np.linspace(-7, 7, 15)
if var_evol_cont_full == 'z_ano': cont_levels = np.arange(-200, 201, 25)  
#if var_evol_cont2_full == 'z_ano': cont_levels2 = np.arange(-200, 201, 25)  
if var_evol_cont3_full == 'blh': cont_levels3 = np.arange(0, 1501, 100) 
if var_evol_cont3_full == 'cape': cont_levels3= [30, 40, 50, 60] #here warnings are displayed since somethimes no contours are found
if var_evol_cont3_full == 'tp': cont_levels3= np.arange(0.2, 1, .2) #[30, 40, 50, 60] #here warnings are displayed since somethimes no contours are found
if 'cc' in var_evol_cont3_full: cont_levels3= np.arange(0.2, 1, .1) #[30, 40, 50, 60] #here warnings are displayed since somethimes no contours are found
...
if imp_list:
    ...
    file= Mediadir + "ERA5_STARS/PL_centred_fields/" +var_evol + '_'+ str(plevel_var) + '_allObs.nc'
    if os.path.isfile(file): ds= xr.open_dataset(file)
    else:
        ds= xr.open_dataset(Mediadir + "ERA5_STARS/PL_centred_fields/" +var_evol + '_all_levs_allObs.nc')
        ds= ds.sel(plev= plevel_var)
    ds= ds.rename({var_evol: var_evol_p})
    
    
    file= Mediadir + "ERA5_STARS/PL_centred_fields/" +var_evol_cont + '_'+ str(plevel_cont) + '_allObs.nc'
    if os.path.isfile(file): ds2= xr.open_dataset(file)
    else:
        ds2= xr.open_dataset(Mediadir + "ERA5_STARS/PL_centred_fields/" +var_evol_cont + '_all_levs_allObs.nc')
        ds2= ds2.sel(plev= plevel_cont)
    
    ds2= ds2.rename({var_evol_cont: var_evol_cont_p})
    ds= xr.merge([ds, ds2])
    
    
#    file= Mediadir + "ERA5_STARS/PL_centred_fields/" +var_evol_cont2 + '_'+ str(plevel_cont2) + '_allObs.nc'
#    if os.path.isfile(file): ds2= xr.open_dataset(file)
#    else:
#        ds2= xr.open_dataset(Mediadir + "ERA5_STARS/PL_centred_fields/" +var_evol_cont2 + '_all_levs_allObs.nc')
#        ds2= ds2.sel(plev= plevel_cont2)
#        ds2= ds2.rename({'plev': 'plev2'})
#    ds2= ds2.rename({var_evol_cont2: var_evol_cont2_p})
#    ds= xr.merge([ds, ds2])
#    
#    
#    
    ds2= xr.open_dataset(Mediadir + "ERA5_STARS/PL_centred_fields/" +var_evol_cont3 + '_allObs.nc')
    ds= xr.merge([ds, ds2])
    ...
    if ano_evol == True:
        ds_Obs_evol[var_evol_p_full]= (('time', 'x', 'y'), ds_Obs_evol[var_evol_p].values- np.mean(ds_Obs_evol[var_evol_p].values, axis= (1,2))[:, np.newaxis, np.newaxis])
    if ano_evol_cont == True:
        ds_Obs_evol[var_evol_cont_p_full]= (('time', 'x', 'y'), ds_Obs_evol[var_evol_cont_p].values- np.mean(ds_Obs_evol[var_evol_cont_p].values, axis= (1,2))[:, np.newaxis, np.newaxis])
#    if ano_evol_cont2 == True:
#        ds_Obs_evol[var_evol_cont2_p_full]= (('time', 'x', 'y'), ds_Obs_evol[var_evol_cont2_p].values- np.mean(ds_Obs_evol[var_evol_cont2_p].values, axis= (1,2))[:, np.newaxis, np.newaxis])
    if ano_evol_cont3 == True:
        ds_Obs_evol[var_evol_cont3_full]= (('time', 'x', 'y'), ds_Obs_evol[var_evol_cont3].values- np.mean(ds_Obs_evol[var_evol_cont3].values, axis= (1,2))[:, np.newaxis, np.newaxis])
#        
...
#        weight_list= 1/Stoll_ind[Stoll_ind.node== iSOM]['K_SOM.distances'].values
    ...
    if sym:
        cf= plt.contourf(ds_node.x, ds_node.y, np.average(ds_node[var_evol_p_full], axis= 0), levels= levels, cmap= cmap, extend='both')
       
        cs= plt.contour(ds_node.x, ds_node.y, np.mean(ds_node[var_evol_cont_p_full], axis= 0), levels= cont_levels, colors='k', linestyles='solid')
#        cs2= plt.contour(ds_node.x, ds_node.y, np.mean(ds_node[var_evol_cont2_p_full], axis= 0), levels= cont_levels2, colors='k', linestyles='dashed')

#        cont_levels3= np.arange(0, 76, 5) #[1000,  1200, 1400]
#        cf= plt.contourf(ds_node.x, ds_node.y, np.average(ds_node[var_evol_cont3_full], axis= 0), levels= cont_levels3, cmap= cmap, extend='both')
        cs3= plt.contour(ds_node.x, ds_node.y, np.average(ds_node[var_evol_cont3_full], axis= 0), levels= cont_levels3, colors= 'g')
    ...
    if max(cont_levels3) <= 1: plt.clabel(cs3, cont_levels3[::2], fontsize=10, fmt='%1.1f')
    else: plt.clabel(cs3, cont_levels3[::2], fontsize=10, fmt='%1.0f')
...
if save:
#    save_name=savedir+ 'SOM_'+var_full+"_"+str(plevel)+"_"+Obs+'--mean_'+var_evol_p_full+"_"+var_evol_cont_p_full +"_"+var_evol_cont2_p_full+"_"+var_evol_cont3_full+'_'+Obs_evol_str+"_x"+str(x)+"_y"+str(y)
    save_name=savedir+ 'SOM_'+var_full+"_"+str(plevel)+"_"+Obs+'--mean_'+var_evol_p_full+"_"+var_evol_cont_p_full+"_"+var_evol_cont3_full +'_'+Obs_evol_str+"_x"+str(x)+"_y"+str(y)
    print(save_name)
    plt.savefig(save_name , bbox_inches='tight')
