...
ID= '73_4' #'25_1' #'95' #'12'  #95
Obs= 15 #starts with 1

Plot_PLCG= False
if Plot_PLCG: save_extra+='_PLCG'
...
#var= 'u'
var='vo'
#contourlevels= [-3, -2, -1, -.5, 0.5, 1, 2, 3]
#var='t'
#var='grad_t'
#var= 'z'
#vmin,vmax= 0, 10
...
#plevel= [1000, 925, 850]
#fignr, plevel= 13, [1000, 850, 500]

#var, gausfilter='baroclinic_dTdy_gr_filter', 4 #calculated by  g/NT dT/dy, 3 plevels: the first and last are used for N and the middle one for T
...
if setup != 'shear':
    if contourlevels != None:
        cf= ax.contourf(ds.lon, ds.lat, ds[var], transform= ccrs.PlateCarree(), cmap= cmap, levels= contourlevels, vmin= contourlevels[0], vmax=contourlevels[-1], extend='both')            
    elif vmax != None:
...
if Plot_Rojo_track:
    ...
    for Rojo_nr in Rojo_nr_list:
        ...
        for track in R_tracks:
            track.plot_track(ax=ax, color='red', label='STARS track')   
...
if Plot_PL_track:
    ax.scatter(S_now.lon, S_now.lat, transform=ccrs.PlateCarree(), s= 50, marker= "s", color= 'g', zorder=2, label='')
    S_now_whole= S.loc[S['ID'] == ID]
    S_now_track= prepare_tracks(S_now_whole, system_id_label="ID")[0]
    S_now_track.plot_track(ax= ax, color='g', label='matched track')

    ax.scatter(S_now_whole.lon, S_now_whole.lat, transform=ccrs.PlateCarree(), s= 50, marker="x", color= 'g', zorder= 2, label='')
...
u_steer= value_in_rad(dsorg['u'].sel(plev= [1000, 700]).mean(axis= 0), ds.lat, ds.lon, center_lat, center_lon, steering_rad)
v_steer= value_in_rad(dsorg['v'].sel(plev= [1000, 700]).mean(axis= 0), ds.lat, ds.lon, center_lat, center_lon, steering_rad)
    
    
beering= UV2Direction(u_steer, v_steer) #wind direction

steer_vel= np.sqrt(u_steer**2+ v_steer**2)
print('steering velocity [m/s]', steer_vel)
steer_lon, steer_lat= gcc.point_given_start_and_bearing((center_lon, center_lat), beering, steer_vel*3.6*steering_time*1E3)
ax.scatter(steer_lon, steer_lat, transform=ccrs.PlateCarree(), s= 50, marker="^", color= 'g', zorder= 2)


#from scipy.interpolate import UnivariateSpline
#spl = UnivariateSpline(S_now_whole.lon, S_now_whole.lat)


import csaps #pip install csaps==0.10.0
#csaps.UnivariateCubicSmoothingSpline(S_now_whole.lon.values, S_now_whole.lat.values, smooth=0.7)
...
sp = csaps.CubicSmoothingSpline(S_t, S_data, smooth= 0.01)
Sm_lon, Sm_lat= sp(S_t)
ax.scatter(Sm_lon, Sm_lat, transform=ccrs.PlateCarree(), s= 50, marker="x", color= 'purple', zorder= 2)
ax.plot(Sm_lon, Sm_lat, transform=ccrs.PlateCarree(), color= 'purple')
ax.scatter(Sm_lon[Obs-1], Sm_lat[Obs-1], transform=ccrs.PlateCarree(), s= 50, marker= "s", color= 'purple', zorder=2)

sp = csaps.CubicSmoothingSpline(S_t, S_data, smooth= 0.1)
Sm_lon, Sm_lat= sp(S_t)
ax.scatter(Sm_lon, Sm_lat, transform=ccrs.PlateCarree(), s= 50, marker="x", color= 'r', zorder= 2)
ax.plot(Sm_lon, Sm_lat, transform=ccrs.PlateCarree(), color= 'r')
ax.scatter(Sm_lon[Obs-1], Sm_lat[Obs-1], transform=ccrs.PlateCarree(), s= 50, marker= "s", color= 'r', zorder=2)
...
#    
#    ax.scatter(center_grid[:,:,0], center_grid[:,:,1], transform=ccrs.PlateCarree(), s= 1, marker="x", color= 'b', zorder= 1)
...
if Plot_PLCG:
    ...
    if contourlevels != None:
        cf= plt.contourf(dist_grid_axis, dist_grid_axis, var_interp, cmap= cmap, levels= contourlevels, vmin= contourlevels[0], vmax=contourlevels[-1], extend='both')            
    elif vmax != None:
    ...
    if save:
        savefile= savedir+ 'StollID'+ID+ '_Obsnr'+str(Obs)+ '_'+var+str(plevel)+'_onPLCG'
        print(savefile)
        plt.savefig(savefile , bbox_inches='tight') 
