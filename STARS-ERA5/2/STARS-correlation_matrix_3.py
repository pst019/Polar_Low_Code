...
if imp_lists:
    ...
    Stoll = merge_ERA5(Stoll, Mediadir, "PL/STARS/Stoll_ERA5_handfix.csv") #the ERA5 merge has to happen with the same stoll list as was used for the production
    ...
    Stoll= Stoll.rename(columns={
       ...
       'skt-t500_max_200': 'SKT -T$_{500}$ (max)', 
       'skt-t700_max_200': 'SKT -T$_{700}$ (max)',
       ...
       'N1000-500_dgauss-4_msk_mean-200': 'N$_{500-1000}$ (mean)',
       'sst-t500_mean_200': 'SST -T$_{500}$ (mean)',
       ...
       'grad_t850_dgauss-4_msk_mean-200': 'Grad T$_{850}$ (mean)',
       ...
       'baroclinic_dTdy_gr1000-925-700_dgauss-4_msk_mean-200': 'LL Baroclinic dT/dy (mean)',
       'baroclinic_dTdy_gr1000-850-500_dgauss-4_msk_mean-200':'UL Baroclinic dT/dy (mean)',
...
#        df= df.rename(columns={'t_PC1':'t_mat_PC1', 't_PC2':'t_mat_PC2', 't_PC3':'t_mat_PC3'})
        ...
        Sx= Sx[ ['Morphology', 'Latitude', #'Cloud diameter', 
                 ...
                 'Vorticity$_{850}$ (centre)', 'Wind Speed 10m (max)',
               'Sea-level pressure (min)', 'Boundary layer height (max)', 'CAPE (max)', 'CAPE (mean)'
#                'Skin temperature (med)',
# 'SKT -T$_{500}$ (max)', 'SKT -T$_{700}$ (max)', 'SST -T$_{500}$ (max)',
                'SST (mean)', 'SST -T$_{500}$ (mean)', 'SST -T$_{700}$ (mean)',
                'N$_{500-1000}$ (min)', 'N$_{700-1000}$ (mean)', 'N$_{500-1000}$ (mean)',
               'Total precip. (mean)', 'Convective precip. (mean)', 'Large-scale precip. (mean)', 'Snow fall (mean)',
               'Sensible heat flux (mean)', 'Latent heat flux (mean)', 
               'Grad T$_{850}$ (max)', 'Grad T$_{850}$ (mean)', #'Baroclinic growth (max)' , #'Barotropic growth (max)',
               'Baroclinic dU/dz (max)' , 'UL Baroclinic dT/dy (max)', 'UL Baroclinic dT/dy (mean)',
               'LL Baroclinic dT/dy (max)', 'LL Baroclinic dT/dy (mean)',
               'Vertical shear angle (mean)', 'Vertical shear strength (mean)',
               't_PC1', 't_PC2', 't_PC3'] ]
