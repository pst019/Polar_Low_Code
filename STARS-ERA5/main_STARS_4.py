...
ID= '162_1'#'10_1' #20_1 (Obs20 for figure)'25_1' #'95' #'12'  #95
Obs= 10 #15 #starts with 1
...
var, clevels='t', list(np.arange(250, 270, 1))
if ano == True: clevels= list(np.arange(-7, 7.1, 1))
    ...
    if len(tracktime_id_now) > 0:
        ...
        for (_, track) in tr.gb:
            ...
            if len(ind_now)== 1:
                ...
                ax.scatter(tracklon, tracklat, transform=ccrs.PlateCarree(), s= 70, marker="s", color= 'b', zorder= 2)
...
if Plot_PL_track:
    ax.scatter(S_now.lon, S_now.lat, transform=ccrs.PlateCarree(), s= 70, marker= "s", color= 'g', zorder=2, label='')
...
#    ax.scatter(Sm_lon, Sm_lat, transform=ccrs.PlateCarree(), s= 70, marker="x", color= 'k', zorder= 2)
    if Plot_smth_track:
        ax.plot(Sm_lon, Sm_lat, transform=ccrs.PlateCarree(), color= 'k', label= 'smoothed track', zorder=1)
...
if Plot_PLCG:
    ...
    if ano == True:
        var_interp= var_interp- np.mean(var_interp)
...
#    cb.set_label(ds[var].long_name + ' ['+ ds[var].units + ']', size=11)
    ...
    plt.xlabel('Distance in propagation direction [km]')
    plt.ylabel('Distance orthogonal to propagation direction [km]')
    
    plt.scatter(0,0,  color='g', s= 70, marker= "s")
    
    
    """make the propagation arrow"""
    arrow_ax = fig.add_axes([0.75, 0.04, 0.12, 0.08])
    ...
    if save:
        savefile= savedir+ 'StollID'+ID+ '_Obsnr'+str(Obs)+ '_'+var
        if ano== True: savefile += 'ano'
        savefile += str(plevel)+'_onPLCG_smth-track'
        print(savefile)
        plt.savefig(savefile , bbox_inches='tight') 
