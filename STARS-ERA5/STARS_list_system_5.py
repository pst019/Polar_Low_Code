...
if imp_lists:
    ...
    Stoll= Stoll_excl(Stoll, lifetime_threshold= 2)
...
"""-----------------------------------------------------------------------------"""
"""set the dataset"""
#Sx, Stype, Sname = STARS_ind, 'system', 'STARS'
#S_total=STARS

Sx, Stype, Sname = Stoll_ind, 'system', 'Stoll'
S_total= Stoll
...
#Sx, Stype, Sname= STARS, 'step', 'STARS'
#Sx, Stype, Sname= Stoll, 'step', 'Stoll'
...
#var, bins= 'tp_mean_200', np.arange(0, .65, .02) #total precip
...
#var, bins= 'Vort_diameter', np.arange(0, 651, 25) #from vorticity area
...
if save:
    savedir= homedir+ 'Polar_Low/STARS-ana/Figs/3/Sub-Morph_'+Sname+'_'+Stype
    print(savedir)
    plt.savefig(savedir) 
...
if var == 'Vort_diameter':
    Sx['Vort_diameter']= np.sqrt(Sx['area']) *4/np.pi
...
#        plt.legend(ncol= 2)
    ...
    elif var=='Diameter': plt.xlabel('Cloud diameter of the polar low [km]')
...
if save:
    ...
    savedir= homedir+ 'Polar_Low/STARS-ana/Figs/3/'+'Morph_'+Sname+'_'+Stype+'_'+var
    print(savedir)
