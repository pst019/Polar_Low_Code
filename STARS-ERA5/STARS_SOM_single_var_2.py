...
lsm_mask=False
size= 500 #250
contour_numbers= False #plot the numbers of the contours in the plot
only_contours=False
...
elif var_full== 'pv_300':  levels = np.arange(2, 6.1, .5)*1E-6
elif var_full== 'pv_500':  levels = np.arange(0.2, 1.3, .1)*1E-6
elif var_full== 'pv_850':  levels = np.arange(0.2, 1.3, .1)*1E-6
...
for iSOM in range(1, x*y +1):
    ...
    if calculate =='mean':
        ds_node_var_calc= np.nanmean(ds_node[var_full], axis= 0)
    if calculate =='std':
        ds_node_var_calc= np.nanstd(ds_node[var_full], axis= 0)
    ...
    if levels_different:
        ...
        cf= plt.contourf(ds_node.x, ds_node.y, ds_node_var_calc, levels= levels[1:-1], cmap= cmap, norm=norm, extend='both')      
        cs= plt.contour(ds_node.x, ds_node.y, ds_node_var_calc, levels= levels[1:-1], colors='k', linestyles='solid')

    elif len(levels) > 0:
        if only_contours== False:
            cf= plt.contourf(ds_node.x, ds_node.y, ds_node_var_calc, levels= levels, cmap= cmap, extend='both')      
            cs= plt.contour(ds_node.x, ds_node.y, ds_node_var_calc, levels= levels, colors='k', linestyles='solid')

        if only_contours== True:
            cs= plt.contour(ds_node.x, ds_node.y, ds_node_var_calc, levels= levels, colors='k')

       
    elif sym:
        cf= plt.contourf(ds_node.x, ds_node.y, ds_node_var_calc, vmin= -vextr, vmax= vextr, cmap= cmap)#, extend='both')      
        cs= plt.contour(ds_node.x, ds_node.y, ds_node_var_calc, colors='k', linestyles='solid')

    else:
        cf= plt.contourf(ds_node.x, ds_node.y, ds_node_var_calc, cmap= cmap, vmin= vmin, vmax= vmax)
        cs= plt.contour(ds_node.x, ds_node.y, ds_node_var_calc, colors='k', vmin= vmin, vmax= vmax)
    ...
    if vo_tend_thresh: plt.title('SOM '+str(iSOM)+ ' ('+str(np.round(perc_of_SOM*100, 1))+'%, '+ str(int(np.round(after_tend_excl/before_tend_excl *100))) + '%)')
    if vo_tend_thresh == False : plt.title('SOM '+str(iSOM)+ ' ('+str(np.round(perc_of_SOM*100, 1))+'%)')
...
if save:
    ...
    save_name+='_size'+str(size)+'--'+calculate+'_'+var_full+"_x"+str(x)+"_y"+str(y)
