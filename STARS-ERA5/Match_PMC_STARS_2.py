...
print('the previous version was used for the manuscript')
...
"""only for Plot= True"""
Plot=True
plot_tracktime_id= 12 #[1, 169], index of the time interval of the ERA5 data
plot_iSTARS= 4 #time index of the Rojo PL
S_now_start, S_now_end= 0, 1 #this choses which of the system occuring now should be taken


Plot_all_tracks_now=False
Plot_Denis_match_methods=False
...
outfile=Mediadir+"ERA5_STARS/STARS-PMC-merge/"+test+"_dist_"+str(dist)+"_2.csv"
...
#    ax= Plot_Polar_Stereo(fig, central_longitude= 30, extent= [-10, 50, 60, 80], subplot= (1,1,1), scalebar=False)
    ax= Plot_Polar_Stereo(fig, central_longitude= 30, extent= [-15, 45, 73, 82], subplot= (1,1,1), scalebar=False)
...
for tracktime_id in tracktime_id_list[tr_id_start: tr_id_end]: #for testing
    ...
    print('tracktime id: ', tracktime_id) #the time block of ERA-5 data used for the tracking
...
#    for Si, S_now_id in enumerate(S_now_idlist):
    if Plot: S_now_idlist=  S_now_idlist[S_now_start:S_now_end]
    ...
    for S_now_id in S_now_idlist:
        ...
        print('S_now_id: ', S_now_id) #the id of the PL event
        ...
        if Plot:
            S_now_track[0].plot_track(ax=ax, color='r', label= 'Rojo track')
            ...
            #            print(tr_i_now)
                ...
                #        if ilegend== 1: ax.scatter(tracklon, tracklat, transform=ccrs.PlateCarree(), s= 50, marker="s", color= 'b', zorder= 2, label= 'Location now')
                        ...
                        ax.text(tracklon, tracklat+ 0.1, track.index.values[0][0], transform=ccrs.PlateCarree(), color='b', fontsize=13, fontweight= 'bold')



        if Plot_Denis_match_methods:
            ...
            if len(matchbs['n']) > 0:
                ...
                if Plot: match_track.plot_track(ax=ax, zorder= 3, color='g', label='BS match')
        ...
        for t_S in range(len(S_now)):
            ...
            for track in tr_tracks: #maybe loop only over intersecting tracks
                ...
                if len(i_track) > 0: #track occurs at this time
                    ...
                    if dist_now <= dist:
                        print('S time', datetime_now,  'Track label', track.index.values[0][0])
                        tr_match+= [track.index.values[0][0] ]
                        
        tr_match= remove_dublicate(tr_match)
        
        if Plot:
            for tr_match_nr in tr_match:
                tr_tracks[tr_match_nr].plot_track(ax=ax, color= 'g', label= 'matched track', zorder= 3)

                tr_i_now = np.argmin(np.abs(tr_tracks[tr_match_nr].time - time_plot_iSTARS).values)
    #            print(tr_i_now)
                tracklon, tracklat= tr_tracks[tr_match_nr].lonlat[tr_i_now]
                ax.scatter(tracklon, tracklat, transform=ccrs.PlateCarree(), s= 50, marker="s", color= 'g', zorder= 2)
                ax.text(tracklon, tracklat+ 0.1, tr_tracks[tr_match_nr].index.values[0][0], transform=ccrs.PlateCarree(), color='g', fontsize=13, fontweight= 'bold')
...
#                    S_now.lon.values[t_S], S_now.lat.values[t_S],  track.lon[ind_track], track.lat[ind_track],
        ...
        match_matrix= np.zeros((len(tr_match), len(tr_match)))
                ...
                if len(overlaptime) == 0:
                    ...
                    if time_diff < 6: #only if the track is intersected for a few hours
                        if len(tr_tracks[tr_match_i]) > time_diff: #the track must at least be as long as the time_diff
                            ...
                            if Plot and Plot_extrapolation: ax.scatter(lon_extr, lat_extr, transform=ccrs.PlateCarree(), s= 50, marker="^", color= 'g', zorder= 2, label='forward extrap')
                        ...
                        if len(tr_tracks[tr_match_j]) > time_diff: #the track must at least be as long as the time_diff
                            ...
                            if Plot and Plot_extrapolation: ax.scatter(lon_extr, lat_extr, transform=ccrs.PlateCarree(), s= 50, marker="v", color= 'g', zorder= 2, label='backward extrap')
...
#        print('STARS: ', S_now_id)
        if Plot== False:
            tr_count_vector= [] #vector to the second PMC that occurs in the match_matix the same tr_count as the first
            ...
            for tr_i, tr_nr in enumerate(tr_match):
                ...
                if len(PMCmatch) == 0:
                    tr_count+=1
                    tr_count_vector+= [tr_count] #no PMC match, so the track count is increased
                else:
                    tr_count_vector+= [tr_count_vector[PMCmatch[0]]] #PMC match is put into the vector
                    
                out_track= tr_tracks[tr_nr][:]
    #            out_track.insert(0, "row_idx", out_track.index.values[1])
                ...
                out_track_pd.insert(0, "row_idx", out_track_pd_0.index.values)
    #            out_track_pd.insert(0, "row_idx", list(out_track.index.values[1])
                merged_track= pd.concat([out_track_pd, STARS_now], axis=1)
                
                merged_track.insert(0, "Stoll nr", S_now_id.split('.')[0]+ '_' + str(tr_count_vector[tr_i]))
...
#        ds['plev']/= 100
    ...
    variable= ds[var].sel(plev= plevel*100 )    
...
#        ax.contour(ds.lon, ds.lat, variable, [1.5], transform= ccrs.PlateCarree(), colors= 'g', linewidths= 1)
#        ax.contour(ds.lon, ds.lat, variable, [2.5], transform= ccrs.PlateCarree(), colors= 'g', linewidths= 2)

        #much faster
#        cf= ax.contourf(ds.lon, ds.lat, variable, transform= ccrs.PlateCarree(), cmap= cmap, vmin= -vextr, vmax= vextr)

        vextr= 6
        cf= ax.contourf(ds.lon, ds.lat, variable, levels= [-4, -3, -2, -1, 1, 2, 3, 4, 6], transform= ccrs.PlateCarree(), cmap= cmap, vmin= -vextr, vmax= vextr, extend='max')
...
if save:
    print(savedir)
    plt.savefig(savedir+ 'ERA_time_int_'+str(plot_tracktime_id)+'_timestep'+str(plot_iSTARS) , bbox_inches='tight') 


print('Number of merged systems:', mergecount)
