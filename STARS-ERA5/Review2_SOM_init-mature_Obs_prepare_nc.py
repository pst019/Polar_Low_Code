...
if Obs == 'mature':
    #just for the mature calculation
    Stoll_ind= Stoll.groupby(['ID']).mean()
    Stoll_ind= calc_Obs_mature(Stoll, Stoll_ind, intensity_var='vo')


#        mature_node= S[S.Obs == Stoll_ind.loc[ind].Obs_mature].node.values[0]
...
#if wind_excl: Stoll_ind= Stoll_ind[Stoll_ind > windthresh]
#ID_int= [int(ID.split('_')[0])*1E3 + int(ID.split('_')[1])for ID in Stoll_ind.index] #makes integer of the PL nrs


"""new: get the IDs for the PLs with multiple matches at a lower distance to Rojo PL"""
#ID= Stoll[110*np.sqrt((Stoll['lat'] - Stoll['Rojo_lat'])**2  + (np.cos(np.deg2rad(Stoll['Rojo_lat']))* (Stoll['lon'] - Stoll['Rojo_lon']))**2 ) < match_dist]
#
#ID_count= ID.groupby('ID').size()
#ID_count[ID_count >= N_occurence]
#
#ID_multiple_match= ID_count[ID_count >= N_occurence].index
#ID_int= [int(ID.split('_')[0])*1E3 + int(ID.split('_')[1])for ID in ID_multiple_match] #makes integer of the PL nrs
...
ds['mature'] = ('time', np.zeros(len(ds['PLnr'])) )

#ds['multi_match']= ('time', np.zeros(len(ds['PLnr']) ) )
...
for PLnow in duration_ds.PLnr.values:
    ...
    PLnow_str= str(int(PLnow//1000)) + '_'+ str(int(PLnow%1000))
    ds['mature'][np.logical_and(ds['PLnr']== PLnow, ds['Obs']==  Stoll_ind['Obs_mature'].loc[PLnow_str]) ] =  1 #label mature with 1

#    if PLnow in ID_int:  ds['multi_match'][ds['PLnr']== PLnow] = 1
...
if Obs == 1:
    ds= ds.where(ds.Obs == 1, drop= True)

if Obs == 'last':
    ds= ds.where((ds.Obs/ds.duration) == 1, drop= True)

if Obs == 'mature':
    ds= ds.where(ds.mature == 1, drop= True)   

print('Only specific Observation:', len(ds.time), 'number systems:', len(ds.groupby('PLnr').last()['PLnr']))

#ds= ds.where(ds.multi_match == 1, drop=True)    
#
#print('Only multimatches: number timesteps:', len(ds.time), 'number systems:', len(ds.groupby('PLnr').last()['PLnr']))
...
if write_nc:
    if PLCG_type== 'stearing_flow':
        ...
        filename= Mediadir + "ERA5_STARS/PL_centred_fields_smooth-tracks/" +var + '_'+ str(plevel) 
        if Obs == 1: filename += '_Obs1'
        elif Obs == 'last': filename += '_Obslast'
        elif Obs == 'mature': filename += '_Obsmature'
        else: filename += '_allObs'
        
        filename += '_SOMprep_track-smth-'+smooth_param
        filename += '_dur'+str(lifetime) #+'_matchdist'+str(match_dist)+'_Nmatches'+str(N_occurence)
#        if wind_excl: filename += '_Umax'+str(windthresh)
        filename += '.nc'
