...
Stoll= pd.concat([Stoll, Stoll_dir_speed, S_nodes, S_ERA5], axis=1)
...
#    ax1.add_patch(arrow)
    ...
    ax1.add_patch(mpatches.FancyArrowPatch((0, 12), (Prop_speed, 12),  mutation_scale=10, color='r', label='Prop') )
...
#    
    ...
    plevel= 925
    wind_beering= UV2Direction(S_SOM['10u_mean-200'], S_SOM['10v_mean-200'])
    rot_beering = wind_beering - S_SOM.prop_dir
#    rot_beering%=360
#    
    U= np.sqrt(S_SOM['10u_mean-200']**2+ S_SOM['10v_mean-200']**2)
#    
    rot_u, rot_v = WindSpeedDirection2UV(U, rot_beering, orientation='' )
        
    ax1.add_patch(mpatches.FancyArrowPatch((0, -10), (rot_v.mean(), -10 -rot_u.mean()),  mutation_scale=10, label= '10m' ))
    ax1.text(-1, -10, '10 m',  horizontalalignment='right', verticalalignment='center')

#    ax1.annotate('u 10m', xy=(rot_v.mean(), -15 -rot_u.mean()), xytext=(0, -15 ),  horizontalalignment='right', verticalalignment='center',  arrowprops=dict(facecolor='k')) #, shrink=0.05))

    
    
    for iplev, plevel in enumerate([925, 850, 700, 500]):
        wind_beering= UV2Direction(S_SOM['u'+str(plevel)+'_mean-200'], S_SOM['v'+str(plevel)+'_mean-200'])
        rot_beering = wind_beering - S_SOM.prop_dir
    #    rot_beering%=360
    #    
        U= np.sqrt(S_SOM['u'+str(plevel)+'_mean-200']**2+ S_SOM['v'+str(plevel)+'_mean-200']**2)
    #    
        rot_u, rot_v = WindSpeedDirection2UV(U, rot_beering, orientation='' )
        ...
        ax1.add_patch(mpatches.FancyArrowPatch((0, -6.5+ iplev*3.5), (rot_v.mean(), -6.5+ iplev*3.5 -rot_u.mean()),  mutation_scale=10, label= str(plevel) ))
        ax1.text(-1, -6.5+ iplev*3.5, str(plevel)+' hPa',  horizontalalignment='right', verticalalignment='center')

        

    ax1.add_patch(mpatches.FancyArrowPatch((0, 10), ( 0.5*(v_700.mean() + v_850.mean()), 10-  0.5*(u_700.mean()+ u_850.mean()) ),  mutation_scale=10, color='r', label='Prop') )
    ax1.text(-.2, 10, 'Est. prop.',  horizontalalignment='right', verticalalignment='center')      
...
#        ax1.annotate(str(plevel), xy=(rot_v.mean(), -10 + iplev*5  -rot_u.mean()), xytext=(0, -10 + iplev*5 ),  horizontalalignment='right', verticalalignment='center',  arrowprops=dict(facecolor='k')) #, shrink=0.05))
    ...
    #    wind_beering= UV2Direction(S_SOM['u500_mean-200'], S_SOM['v500_mean-200'])
    #    rot_beering = wind_beering - S_SOM.prop_dir
    #    rot_beering%=360
    ##    
    #    U= np.sqrt(S_SOM['u500_mean-200']**2+ S_SOM['v500_mean-200']**2)
    #    
#        rot_u, rot_v = WindSpeedDirection2UV(U, rot_beering, orientation=''  )
#            
...
fig.text(0.5, 0.06, r"Wind in propagation direction [m$\cdot$s$^{-1}$]", ha='center')
fig.text(0.08, 0.5, 'Wind perpendicular to propagation [m$\cdot$s$^{-1}$]', va='center', rotation='vertical')

#fig.tight_layout()

    
#    rot_u.mean(), rot_v.mean()
##    mean_speed= speed_925.mean
#()
    

#
if save:
    save_name=savedir+ 'SOM_Wind_vectors'
