...
def PlotContours(Lon, Lat, psurf, map, nrlevels=10, leveldist=None,levels=None, numbers=True, color= 'k', alpha=1):
    ...
    if levels is not None:
        ...
        cs= map.contour(Lon, Lat, psurf, nrlevels, linewidths= 1. , colors= color, alpha= alpha)#, colors= 6*['b']+ 6*['r'],)
    if numbers == True: plt.clabel(cs, fontsize=10, inline=1, fmt='%1.0f', colors= 'black')
    #plt.tight_layout()
...
def PlotColorMap4(Lon, Lat, data, map, maxlevel= None, symetric=False, bounds=None, bounddist=None, ticks='some', label='', color= 'viridis', boxnr= 14):
    """ plot a color map, e.g. vertical velocity
    if symetric == True it is symetric around 0 and the maxlevel is calculated automatically
...
#        print('change sign')
    ...
    if arrowtype== 'barb':
        ...
        if alen != None:
            Q = map.quiver(Lon,Lat,u,v, pivot= 'mid', scale=20* alen)
            qk = plt.quiverkey(Q, 0.22, 0.97, alen, str(alen)+' m/s', labelpos='W')
...
def PlotWind2(lon, lat, u, v, map, everyx= 1, everyy=1, rot=True, arrowtype= 'quiver', alen=20, scale=None, color='k', colormap=False, option= None):
    """ winds with barbs
...
def PlotCross_sec_T(press, lon, field, levels= 10):#, color= 'k'):
    """now for temp
...
#    if color== 'default': color= [(plt.cm.viridis(h)) for h in range(256)]
    ...
    cs= plt.contour(lon, press , field, levels, linewidths= 1.)# , colors= color )
    plt.clabel(cs, fontsize=10, inline=1, fmt='%1.0f', color= 'black')
