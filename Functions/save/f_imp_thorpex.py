...
class data:
    def __init__(self, year, month, day, hour, level='vertical', filedir=Mediadir+'PL/IPY-THOTPEX_DROPSONDES_3-4-MARCH_2008/QC_',
                 plevels= np.arange(1000, 250, -10), dropid= 1, exclude=[]):
        """import Thorpex data
        ...
        if level is 'vertical':
            self.get_vertical(times, filedir, exclude)
    ...
    def get_vertical(self, times, filedir, exclude):
...
#            Ti= griddata(presr, Tr, self.plevels)
            ...
            if np.max(pdiff) > 300: #exclude this data
