...
#        else:
    ...
    def imp_standard(self):
        """import all levels for the most important data -from fp"""
        nc= Dataset(self.filename)
    ...
    def imp_standard_t(self, tn):
        """import all levels of the most important data for timestep tn -from fp"""
        nc= Dataset(self.filename)

        # Variables
        self.PV = nc.variables['ertel_potential_vorticity_pl'][tn, :][::-1, ::self.res, ::self.res]
        self.geop = nc.variables['geopotential_pl'][tn, :][::-1, ::self.res, ::self.res]/9.81
        
#        self.T = nc.variables['air_temperature_pl'][:][:, ::-1, ::self.res, ::self.res]
#        self.mslp = nc.variables['air_pressure_at_sea_level'][:, 0][:, ::self.res, ::self.res]/100 #in hPa
        self.u = nc.variables['x_wind_pl'][tn, :][::-1, ::self.res, ::self.res] #
        self.v = nc.variables['y_wind_pl'][tn, :][::-1, ::self.res, ::self.res]
...
#        print(nc.variables.keys())
    ...
    def imp_geop1000(self, tn):
        nc= Dataset(self.filename)
        self.geop1000 = nc.variables['geopotential_pl'][tn, -1][::self.res, ::self.res]/9.81
     

    def imp_sfx(self):
        nc= Dataset(self.filename)
        self.SST = nc.variables['SST'][0, 0][::self.res, ::self.res]
...
#        print(nc.variables.keys())
        ...
        self.CAPE = nc.variables['specific_convective_available_potential_energy_old'][tn, 0][::self.res, ::self.res]  #J/kg
        self.CIN = nc.variables['atmosphere_convective_inhibition_old'][tn, 0][::self.res, ::self.res] #J/kg
        ...
        self.prec_acc = nc.variables['precipitation_amount_acc'][tn, 0][::self.res, ::self.res]       
    ...
    def imp_atm_3h(self, tn):
        """import data that is only available for one level for the atmosphere -for fp
        3hourly accumulation until tn"""
        nc= Dataset(self.filename)
#        print(nc.variables.keys())
...
#                               -nc.variables['integral_of_surface_downward_latent_heat_sublimation_flux_wrt_time'][tn+1, 0][::self.res, ::self.res] + nc.variables['integral_of_surface_downward_latent_heat_sublimation_flux_wrt_time'][tn, 0][::self.res, ::self.res] )/(60*60) #upward W/m**2       
#        self.SenH = (-nc.variables['integral_of_surface_downward_sensible_heat_flux_wrt_time'][tn+1, 0][::self.res, ::self.res] + nc.variables['integral_of_surface_downward_sensible_heat_flux_wrt_time'][tn, 0][::self.res, ::self.res] )/(60*60) #upward W/m**2      
        self.OLW = (-nc.variables['integral_of_toa_outgoing_longwave_flux_wrt_time'][tn, 0][::self.res, ::self.res] + nc.variables['integral_of_toa_outgoing_longwave_flux_wrt_time'][tn-3, 0][::self.res, ::self.res] )/(3*60*60) #upward W/m**2      
...
#        self.mslp = nc.variables['air_pressure_at_sea_level'][tn, 0][::self.res, ::self.res]/100 #in hPa
    ...
    def imp_cross_sec(self, xn, yn, tn):
        ...
        #maybe not necessary again...
        if 'pressure' in nc.variables:  
            self.pres= nc.variables['pressure'][:][::-1]
        elif 'pressure0' in nc.variables:
            self.pres= nc.variables['pressure0'][:][::-1]
...
#        print(nc.variables.values())
    ...
    def imp_cross_sec_grib(self, xn, yn, tn):
        ...
        print('AROME surface pressure ', ps)
#        self.pres= (ap.reshape([65, 1, 1]) + b.reshape([65, 1,1])* ps )[::-1] /100 #hPa
        self.pres= (ap.reshape([65]) + b.reshape([65])* ps )[::-1] /100 #hPa


    def imp_cross_sec_grib_2(self, xn, yn, tn):
        """import data for a cross section following xn and yn at time tn -for fp
        xn and yn from d.imp_cross_sec
        I rotated xn and yn - maybe this introduced an error somewhere
        used in main_AROME_5 - the version before did not work- this is more trustworthy"""
        filename= self.filename.replace('_fp', '')
        nc= Dataset(filename)
        ...
        ps= nc.variables['surface_air_pressure'][tn][:, ::self.res, ::self.res][:, xn, yn]
#        print('AROME surface pressure ', ps)
        self.pres= (ap.reshape([65, 1]) + b.reshape([65, 1])* ps )[::-1]/100 #hPa

    def imp_uv_ft(self, pn, additional='no'):
        """import the vorticity of level pn for the full time period
        additional ['no', 'all' - import all additionals, List of parameters (e.g. [T, SH]) ]"""     
#        print('presslevel: '+ str(self.pres[pn]) )
        ...
        if additional== 'all' or 'T' in additional:
            self.T_ft = nc.variables['air_temperature_pl'][:, pn][:, ::self.res, ::self.res]
        if additional== 'all' or 'SH' in additional:
            self.SH_ft = nc.variables['specific_humidity_pl'][:, pn][:, ::self.res, ::self.res]                        
        if additional== 'all' or 'prec' in additional:
            self.prec_ft = (nc.variables['precipitation_amount_acc'][1:, 0][:, ::self.res, ::self.res] - nc.variables['precipitation_amount_acc'][:-1, 0][:, ::self.res, ::self.res] )  #mm/h
        if additional== 'all' or 'LatH' in additional:
            self.LatH_ft = (-nc.variables['integral_of_surface_downward_latent_heat_evaporation_flux_wrt_time'][1:, 0][:, ::self.res, ::self.res] + nc.variables['integral_of_surface_downward_latent_heat_evaporation_flux_wrt_time'][:-1, 0][:, ::self.res, ::self.res] +
                            -nc.variables['integral_of_surface_downward_latent_heat_sublimation_flux_wrt_time'][1:, 0][:, ::self.res, ::self.res] + nc.variables['integral_of_surface_downward_latent_heat_sublimation_flux_wrt_time'][:-1, 0][:, ::self.res, ::self.res])/(60*60) #upward W/m**2 
        if additional== 'all' or 'SenH' in additional:                                     
            self.SenH_ft = (-nc.variables['integral_of_surface_downward_sensible_heat_flux_wrt_time'][1:, 0][:, ::self.res, ::self.res] + nc.variables['integral_of_surface_downward_sensible_heat_flux_wrt_time'][:-1, 0][:, ::self.res, ::self.res])/(60*60) #upward W/m**2      
    ...
    def imp_grib(self, tn, calcheight=False):
        """import data for at time tn for all model levels
        calcheight is translating the model pressure to model height [m]"""
        filename= self.filename.replace('_fp', '')
        nc= Dataset(filename)

        # Variables
        self.T = np.array(nc.variables['air_temperature_ml'][tn][::-1, ::self.res, ::self.res])
#        self.u = nc.variables['x_wind_ml'][tn][::-1, ::self.res, ::self.res][:, xn, yn]
#        self.v = nc.variables['y_wind_ml'][tn][::-1, ::self.res, ::self.res][:, xn, yn]       
#        self.SH = nc.variables['specific_humidity_ml'][tn][::-1, ::self.res, ::self.res][:, xn, yn]
        ...
        ap= nc.variables['ap'][:]
        b= nc.variables['b'][:]
        ps= nc.variables['surface_air_pressure'][tn][0, ::self.res, ::self.res]
#        print('AROME surface pressure ', ps)
#        self.pres= (ap.reshape([65, 1]) + b.reshape([65, 1])* ps )[::-1]/100 #hPa
#        print(b.shape, ps.shape)
        self.pres= np.array(ap[:, np.newaxis, np.newaxis] + b[:, np.newaxis, np.newaxis]* ps[np.newaxis,:,:] )[::-1]/100 #hPa

        if calcheight:
            R, g= 287.05, 9.81
            H= R* self.T/g
            self.dz= (H[1:] +H[:-1])/2 * np.log(self.pres[:-1]/self.pres[1:])
            
#            z= 11* np.ones(np.shape(self.pres)) #11 is the level of the lower model level
            self.z= np.array([11 + np.sum(self.dz[:i], axis= 0) for i in range(len(b))])
