...
#class NCVar(object):
#    def __init__(self,sdate='20090101',edate='20181231',freq='6h',\
#                 ncvar='MSL',resolution='0.75deg',dset='erai',\
#                 latlim=50.,FillValue=0.):
...
class NCVar(object):
    def __init__(self, file_prefix='surface', ncvar='msl', dset='era5',
                 syear='1999', smonth='12', sday='19', eyear='1999', emonth='12', eday='19',
                 latlim=50., FillValue=0.,
                 datapath= "/media/pst019/1692A00D929FEF8B/ERA5_STARS/data/"):
#        self.dset, self.sdate, self.edate = dset, syear, smonth, s, edate
#        self.freq, self.resolution = freq, resolution
        self.bn = "%s%s_%s_%s_%s_%s" % \
                   (datapath, file_prefix, dset, syear, smonth, sday)
        print(self.bn)
        self.ncvar = ncvar
        self.fn = "%s.nc" % self.bn
        self.latlim = latlim
...
#        print(time.units)
#        print(nc.num2date(fp.variables['time'][:], fp.variables['time'].units))
...
#        return [self.cdftime.num2date(t) for t in time[:]]
        return nc.num2date(time[:], time.units)
    ...
    def readData(self):
        ...
        latidx = np.where(lat>=0.)
        #latidx = np.where(lat>=self.latlim)
        self.lat = lat[latidx]
        self.lon = np.array(fp.variables['lon'][:])
        self.d = np.ma.array(fp.variables[self.ncvar][:,latidx[0],:])
        fp.close()

    def getMapParams(self):
        parallels = np.arange(-90.,90.,10.)
        meridians = [0,45,90,135,180,225,270,315]
        m = Basemap(resolution='c',area_thresh=100000.,projection='npstere',\
                    boundinglat=self.latlim,lon_0=0.,lat_ts=90.)
    ...
    def outputNetCDF(self):
        ...
        for didx, date in enumerate(self.dates):
            ...
            if didx==0:
                fp = nc.Dataset(self.bn+'_eag.nc','w',format='NETCDF3_CLASSIC')
...
#            time[didx] = self.cdftime.date2num(date)
            time[didx]= nc.date2num(date, self.timeunits, self.calendar)
            vari[didx,:,:] = field
...
#    slp = NCVar(sdate='20090101',\
#                edate='20181231',\
#                ncvar='MSL')
    slp= NCVar(file_prefix='surface', ncvar='msl', dset='era5',
                 syear='1999', smonth='12', sday='19', eyear='1999', emonth='12', eday='19',
                 latlim=50., FillValue=0.)
