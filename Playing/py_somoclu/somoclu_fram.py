...
# Reading data
d = xr.open_dataset(f'{datadir}{fname}', chunks = {'time': 38})
...
orig_x = d.shape[1]
orig_y = d.shape[2]
d = d.reshape((d.shape[0], orig_x*orig_y))
print(d.shape)
...
fig.savefig('som_SMB_map_100e.eps')
fig.savefig('som_SMB_map_100e.png')


np.save(f'{datadir}umat.npy',som.umatrix)
np.save(f'{datadir}bmus.npy',som.bmus)
np.save(f'{datadir}codebook.npy',som.codebook)
