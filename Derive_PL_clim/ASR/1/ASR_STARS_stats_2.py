...
#            statU500 += [np.max(np.sqrt(d.u500[maskc]**2 + d.v500[maskc]**2))]     
            ...
            statpdiff1 += [d.SLP[i, pts2D[0][i], pts2D[1][i] ] - np.mean(d.MSLP[maskc]) ]
            statpdiff2 += [d.SLP[i, pts2D[0][i], pts2D[1][i] ] - np.mean(d.MSLP[maskc2]) ]
            statpdiff3 += [d.SLP[i, pts2D[0][i], pts2D[1][i] ] - np.mean(d.MSLP[maskc3]) ]            
            statpdiff5 += [d.SLP[i, pts2D[0][i], pts2D[1][i] ] - np.mean(d.MSLP[maskc5]) ]
