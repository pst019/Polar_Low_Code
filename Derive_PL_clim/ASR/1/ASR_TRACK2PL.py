...
vortlim= 3.8 #the higher the less PLs
thetastablim= np.float64(-12.5) #the higher the less PLs

windlim= np.float64(16.7) #the higher the less PLs
polarfrontlim= np.float64(29.7) #the higher the more PLs
...
#                londist2= int(latdist2//np.cos(np.deg2rad(PLlist[3][i])))
                mask2= radMask2D(d.SST.shape[1:], (int(pts2D[0]), int(pts2D[1])), radiusx= latdist, radiusy= latdist) #lat lon mask
                maskc2= [t, mask2] #tim, lat, lon mask (mask complete)
                SSTlist[i]= np.mean(d.SST[maskc2])

#        PLlist= np.vstack((PLlist, Ulist))
        PLlist = PLlist[:, SSTlist > 3/4* 272]
        ...
        for day in range(1, daynr_month(year, month)+1):
            ...
            for i in index:
                ...
                latdist= 5*55/15
#                londist2= int(latdist2//np.cos(np.deg2rad(PLlist[3][i])))
                mask2= radMask2D(d.u10.shape[1:], (int(pts2D[0]), int(pts2D[1])), radiusx= latdist, radiusy= latdist) #lat lon mask
                maskc2= [t, mask2] #tim, lat, lon mask (mask complete)
...
#                londist= int(latdist//np.cos(np.deg2rad(PLlist[3][i])))
                mask= radMask2D(d.SST.shape[1:], (int(pts2D[0]), int(pts2D[1])), radiusx= latdist, radiusy= latdist) #lat lon mask
    ...
    WritePLlistComp2S(cPLlist, year, name='3', model='ASR')
