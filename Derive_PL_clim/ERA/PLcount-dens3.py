...
importdata= True #import and filter

hemisp= 'NH'
#hemisp = 'SH' #have to change it manually in some places

intense=True
pdiffintense= -.71 #hPa, values are for SLP - mean(SLP), so negative
stabtheta500intense= -4.4 

"""import the data"""
if importdata == True:
    countdens= np.zeros(((eyear-syear), 12, 110, 720))
    ...
    for y, year in enumerate(range(syear, eyear)):
        for month in range(1, 13):
            TPL=TRACK_PLlist_2(year, month, hemi= hemi, name=Tname)
            ...
            if intense== False:
                ...
                PLlist= TPL.PLlist[:, np.logical_and.reduce((TPL.PLlist[-1] == 1, TPL.PLlist[7] <-pdiffintense, TPL.PLlist[5] > stabtheta500intense))]                
            
            latraw= np.array(np.round(-(PLlist[3]-d.lat[0])*2), dtype= int)
            lonraw= np.array(np.round((PLlist[2] - d.lon[0])*2), dtype= int)%720
                                        
            for i in range(np.shape(PLlist)[1]):
                countdens[y, month-1, latraw[i], lonraw[i]] +=1
    
    #        """regions of same shear"""
    #        for i in remove_dublicate(PLlist[0]):
    #            shear += [TPL.PLlist[-2, TPL.PLlist[0]== i][0]]
    #            latpos += [latraw[PLlist[0]== i][0]]
    #            lonpos += [lonraw[PLlist[0]== i][0]]
                
    #latrad= 4 #in pixel so 4 pixel are 2degree in latitude, being about 220km
    #areafact= latrad**2/np.cos(np.deg2rad(d.lat))
...
slope, intercept, r_value, p_value, std_err = stats.linregress(bins, adistrNordic/4)
print('trend: ', slope, 'pvalue of trend:', p_value)
...
#    PlotIce(Lon, Lat, Ice, map)
#    
#    plt.savefig('/home/'+user+'/home/Polar_Low/Documents/ERA-PLs/Graphs/Countd/'+str(month)+'.png', dpi= 50)
