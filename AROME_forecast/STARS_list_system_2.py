...
if save:
    savedir= homedir+ 'Polar_Low/Arome-Arctic-operational/STARS-ana/Sub-Morph'
    print(savedir)
    plt.savefig(savedir)            
...
if var != 'Prop_dir':
    ...
    for morph in pd.crosstab(S_ind[var], S_ind.PL_Morph).columns:
        botn,b_dn,p_dn= plt.hist(S_ind[S_ind['PL_Morph'] == morph][var], bins= bins , label=morph, bottom= bot, edgecolor= 'k', lw= 1)
        bot += botn
    #
...
#        print(mi, morph, col, row)
        ...
        ax.bar(S_ind[S_ind['PL_Morph'] == morph]['Prop_dir'], S_ind[S_ind['PL_Morph'] == morph]['Prop_speed'], normed=False, bins=np.arange(0, 61, 20),  opening=0.9, edgecolor='white')
