...
var, bins= 'barotropic850_max_200', np.arange(0, 25.1, 1)

#var, bins= 'Latitude', np.arange(51, 82.1, 1) #55, 82
#var, bins= 'start_lat', np.arange(51, 82.1, 1) #58, 82
#var, bins= 'end_lat', np.arange(51, 82.1, 1)
#var, bins= 'Longitude', np.arange(-30, 70.1, 5) #-30, 55.1
...
if save:
    savedir= homedir+ 'Polar_Low/Arome-Arctic-operational/STARS-ana/2/Sub-Morph_'+Stype
    print(savedir)
    plt.savefig(savedir) 
...
if var in ['Duration', 'Prop_speed', 'Prop_dir']:
    ...
    Sx= Sx.rename(columns={"datetime":"starttime"})
    ...
    Sx= Sx.rename(columns={"datetime":"endtime"})   
...
if var in ['Distance', 'Prop_speed', 'Prop_dir', 'start_lat', 'start_lon', 'end_lat', 'end_lon']:
    Sx= Sx.rename(columns={"Latitude":"avg_lat", "Longitude":"avg_lon"})
    
    startloc= S.loc[S['Obs'] == 1].groupby(['ID']).first()[['Latitude', 'Longitude']]
    Sx= pd.concat([Sx, startloc], axis= 1, sort=True)
    Sx= Sx.rename(columns={"Latitude":"start_lat", "Longitude":"start_lon"})
    ...
    Sx= Sx.rename(columns={"Latitude":"end_lat", "Longitude":"end_lon"})
...
#    Sx.reset_index(inplace=True, drop=True)
    if system_char== 'max': Sx[var]= S.groupby(['ID']).max()[var]
...
#S.loc[S['Obs'] == 1].groupby(['ID']).first()['datetime']
...
if save:
    ...
    savedir= homedir+ 'Polar_Low/Arome-Arctic-operational/STARS-ana/2/'+'Morph_'+Stype+'_'+var
    print(savedir)
