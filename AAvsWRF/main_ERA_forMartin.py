...
else: plt.figure(fignr, figsize= (5, 4.5))
...
for day in np.arange(14, 22):
    for hour in [0, 12]:
        ...
        lllon, lllat, urlon, urlat= -8, 65, 65, 80
        lat0, lon0= 70, 10 #(lat0, lon0) = center point
        ...
        if var == 'PressWind_Martin':
            PlotContours(Lon, Lat, d.MSLP[0], map, leveldist= 4, numbers=True)
        
            d.imp_u10()
            U10= np.sqrt(d.u10[0]**2+d.v10[0]**2)
            PlotWindVelo(Lon, Lat, U10, map, Umax= 25, color='YlBu')
            PlotWind(d.lon, d.lat, d.u10[0], d.v10[0], map, nx= 35, ny= 70, scale= 300)
        ...
        if save== True:
            ...
            plt.savefig(savename+'.pdf', pad_inches=0)
            print(savename)