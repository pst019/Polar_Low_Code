...
#    plt.legend(loc='upper left')
    ...
    plt.plot(thor.U, thor.pres, 'black', label='Drop', lw=0.5)   
    plt.plot(np.sqrt(AA.u**2 + AA.v**2), AA.pres, 'r', label='AA', lw= 0.5)
    HRESU= np.sqrt(HRES_x.u**2 + HRES_x.v**2)
    plt.plot(HRESU, HRES_x.plev, 'g', label='HRES', lw= 0.5)  
...
#    plt.setp(ax4.get_yticklabels(), visible=False)
    ...
    plt.quiver(thorinterpU, plevels0, thorinterpu/thorinterpU, thorinterpv/thorinterpU, scale= 7, width= 0.02, color='black', pivot='mid')
...
#    PlotColorMap4(Lon[1:], Lat[1:], dir_rot, map)
    ...
    Q= plt.quiver(AAinterpU, #np.ones(len(plevels0))*1,
                  plevels0, AAinterpu/AAinterpU, AAinterpv/AAinterpU, scale= 7, width= 0.02, color= 'r', pivot='mid')
