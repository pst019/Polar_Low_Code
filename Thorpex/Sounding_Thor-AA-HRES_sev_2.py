...
#if sound in [1,2]:
#    HRES_name_0="oper_SFC_200803"+str(HRESfileday).zfill(2)+'_'+str(HRESfilehour).zfill(2)+".nc"
#    HRES_0= xr.open_dataset(Mediadir+'ECMWF/HRES/'+HRES_name_0)
#    HRES['plev']= HRES['hyam'] + HRES['hybm']*HRES_0['msl'][lacktime]
#
#else:
...
#if sound== 1:
#    HRESx= HRES.sel(time=HRES.time.values[0], lat= xr.DataArray(thorlat, dims='location'), lon= xr.DataArray(thorlon, dims='location'),
#                latitude= xr.DataArray(thorlat, dims='location'), longitude= xr.DataArray(thorlon, dims='location'),
#                method='nearest' )
#else:
HRESx= HRES.sel(time=HRES.time.values[0], lat= xr.DataArray(thorlat, dims='location'), lon= xr.DataArray(thorlon, dims='location'), method='nearest' )


#the interpolation is a bit wrong since it takes the average plevel of the dropsondes instead of every individual - a loop could solve this
HREST= interp1d(np.average(HRESx['plev'], axis= -1), HRESx['t'], axis= 0)(plevels)
HREStheta = potential_temperature(np.tile(plevels, (HREST.shape[1],1)).T*units.hPa, HREST* units.K)

HRESu=interp1d(np.average(HRESx['plev'], axis= -1), HRESx['u'], axis= 0)(plevels)
HRESv=interp1d(np.average(HRESx['plev'], axis= -1), HRESx['v'], axis= 0)(plevels)
HRESU= np.sqrt(HRESu**2 + HRESv**2)

HRESSH= interp1d(np.average(HRESx['plev'], axis= -1), HRESx['q'], axis= 0)(plevels)
HRESRH= relative_humidity_from_specific_humidity(HRESSH, HREST* units.K, np.tile(plevels, (HREST.shape[1],1)).T*units.hPa)
#HRES
...
gs = gridspec.GridSpec(1, 4, width_ratios=[3, 3, 3, 2]) 
fignr +=1
plt.clf()
...
plt.plot(Thorthetaavg, plevels, 'black', label='Drop', lw= 2)


plt.plot(AAthetaavg, plevels, color='r', label='AA', lw= 2)# +'+str(hour-filehour))
if exp2== True: plt.plot(AAthetaavg2, plevels, color='g', label='Arome +'+str(hour-filehour2), lw= 2)


#HRESthetaavg= mpcalc.potential_temperature(HRESx.plev.values* units.hPa, HRESx.t.mean(dim='location').values * units.K)
#HRESthetaavg= mpcalc.potential_temperature(HRESx.plev.values* units.hPa, HRESx.t.mean(dim='location').values * units.K)

plt.plot(np.average(HREStheta, axis= 1), plevels, label='HRES', color='g', lw= 2)
...
plt.plot(np.average(thorRH, axis= 0), plevels, 'black', label='Drop', lw= 2)

plt.setp(ax2.get_yticklabels(), visible=False)
#theta= potential_temperature(AA.pres*units.hPa, AA.T*units.K)

plt.plot(AARHavg, plevels, 'r', label='AA', lw= 2)
if exp2== True: plt.plot(AARHavg2, plevels, color='g', lw= 2, label='AA')#+'+str(hour-filehour2))

plt.plot(np.average(HRESRH, axis=1), plevels, label='HRES', color='g', lw= 2)
...
plt.plot(ThorUavg, plevels, 'black', label='Thor', lw= 2)#, lw=0.5)



plt.plot(AAUavg, plevels, 'r', label='Arome', lw= 2)#, lw=0.5)

if exp2== True:
    AAUavg2 = np.sqrt(AAuavg2**2 + AAvavg2**2)
    plt.plot(AAUavg2, plevels, color='g', label='Arome +'+str(hour-filehour2), lw= 2)
    plt.quiver(AAUavg2[::40], plevels[::40], AAuavg2[::40]/AAUavg2[::40], AAvavg2[::40]/AAUavg2[::40], scale= 7, width= 0.02, color='g', pivot='mid')

HRESUavg= np.average(HRESU, axis=1)
plt.plot(HRESUavg, plevels, label='HRES', color='g', lw= 2)#, lw=0.5 )
...
plt.quiver([0]*len(plevels[::40]), plevels[::40], Thoruavg[::40]/ThorUavg[::40], Thorvavg[::40]/ThorUavg[::40], scale= 3, width= 0.025, color='black', pivot='mid')
plt.quiver([1]*len(plevels[::40]), plevels[::40], AAuavg[::40]/AAUavg[::40], AAvavg[::40]/AAUavg[::40], scale= 3, width= 0.025, color='r', pivot='mid')

HRESuavg= np.average(HRESu, axis=1)
HRESvavg= np.average(HRESv, axis=1)

plt.quiver([2]*len(plevels[::40]), plevels[::40], (HRESuavg/HRESUavg)[::40], (HRESvavg/HRESUavg)[::40], scale= 3, width= 0.025, color='g', pivot='mid' )
...
plt.plot(AAthetaavg- Thorthetaavg, plevels, 'r', label='BIAS', lw= 2)
plt.plot(np.average(np.abs(AAtheta - thortheta*units.K), axis= 0), plevels, 'r', label= 'MAE', linestyle='dashed')
plt.plot(-np.average(np.abs(AAtheta - thortheta*units.K), axis= 0), plevels, 'r', linestyle='dashed')

plt.plot(np.average(HREStheta, axis=1) - Thorthetaavg, plevels, 'g', label='BIAS', lw= 2)
plt.plot(np.average(np.abs(HREStheta.T - thortheta*units.K), axis= 0), plevels, 'g', label= 'MAE', linestyle='dashed')
plt.plot(-np.average(np.abs(HREStheta.T - thortheta*units.K), axis= 0), plevels, 'g', linestyle='dashed')
...
plt.plot(np.average(AARH, axis=0)- np.average(thorRH, axis= 0), plevels, 'r', label='BIAS', lw= 2)
plt.plot(np.average(np.abs(AARH - thorRH), axis= 0), plevels, 'r', label= 'MAE', linestyle='dashed')
plt.plot(np.average(-np.abs(AARH - thorRH), axis= 0), plevels, 'r', linestyle='dashed')

plt.plot(np.average(HRESRH, axis=1)- np.average(thorRH, axis= 0), plevels, 'g', label='BIAS', lw= 2)
plt.plot(np.average(np.abs(HRESRH.T - thorRH), axis= 0), plevels, 'g', label= 'MAE', linestyle='dashed')
plt.plot(np.average(-np.abs(HRESRH.T - thorRH), axis= 0), plevels, 'g', linestyle='dashed')
...
plt.plot(np.average(AAU, axis= 0)- np.average(thorU, axis= 0), plevels, 'r', label='AA BIAS', lw= 2)
plt.plot(np.average(np.abs(AAU - thorU), axis= 0), plevels, 'r', label= 'AA MAE', linestyle='dashed')
plt.plot(-np.average(np.abs(AAU - thorU), axis= 0), plevels, 'r', linestyle='dashed')

plt.plot(np.average(HRESU, axis= 1)- np.average(thorU, axis= 0), plevels, 'g', label='HRES', lw= 2)
plt.plot(np.average(np.abs(HRESU.T - thorU), axis= 0), plevels, 'g', linestyle='dashed')
plt.plot(-np.average(np.abs(HRESU.T - thorU), axis= 0), plevels, 'g', linestyle='dashed')

ax3.axvline(x=0, color='grey', linestyle='dashed')
plt.legend(loc= 2)
