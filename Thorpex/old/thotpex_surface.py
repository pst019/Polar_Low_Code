...
#    #Udir = nc.variables['wdir'][:]#wind direction
    ...
    timr = nc.variables['time'][:][~Ur.mask]
    latr = nc.variables['lat'][:][~Ur.mask]
    lonr = nc.variables['lon'][:][~Ur.mask]
    altr= nc.variables['alt'][:][~Ur.mask]
    
    # Variables
    Tr = nc.variables['tdry'][:][~Ur.mask]
    presr = nc.variables['pres'][:][~Ur.mask]
    ur = nc.variables['u_wind'][:][~Ur.mask]
    vr = nc.variables['v_wind'][:][~Ur.mask]
    #U = sqrt(u10**2+v10**2)

    #Udir = nc.variables['wdir'][:]#wind direction
    thetar= nc.variables['theta'][:][~Ur.mask] #potential temperature
    Ur = Ur[~Ur.mask]
...
#    alt[t]= altr[np.where(altr < 30)][0]
    ...
    """condition for 920 hPa"""
    tim[t]= timr[presr > 920][-1]
    lat[t]= latr[presr > 920][-1]
    lon[t]= lonr[presr > 920][-1]
    alt[t]= altr[presr > 920][-1]
    pres[t]= presr[presr > 920][-1]
    U[t]= Ur[presr > 920][-1]
    theta[t]= thetar[presr > 920][-1]
...
theta=theta[np.isnan(lat)== False][np.isnan(alt)== False]
