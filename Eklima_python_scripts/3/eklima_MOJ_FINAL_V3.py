    ...
    while dstart < dto:
        ...
        for obs in obsroot.findall('item'):
            ...
            for st in obs.find('location').findall('item'):
                ...
                reported.add(stnr)
                print(remain_st, type(remain_st))
                break
                #do some debugging here - maybe compare to the python2 code that is working
                print(remain_st.index, )
...
def _pretty_print(filename, stations, dates=[], data=[]):
    ''' Pretty print the data for ONE element of the data array returned by _get_data() '''
    ...
    for stnr in stnrs:
        ...
        if demd == '0': 
            ...
            stations[stnr]['estd'], demd) ).encode('utf8')  )
    if len(dates) > 0:
        f.write('#\n')
        f.write('# Weather data:\n')
#        f.write('# Datetime      \t%9s\n' % '\t'.join(map(lambda x: '%9s' % x, data[first].keys())) )
        f.write('# Datetime      \t%9s\n' % '\t'.join(['%9s' % x for x in data[first].keys()]) )

        for date in dates: 
#            f.write('%s\t%s\n' % (__strftime_prettyprint(date), 
#                '\t'.join(map(lambda x: '%9.1f' % x, data[date].values())) )  )
            f.write('%s\t%s\n' % (__strftime_prettyprint(date), 
                '\t'.join(['%9.1f' % x for x in data[date].values()]) )  )
    f.close()
...
#            date.hour, date.minute, date.second], dtype='f8'), dates)
    matdates = [np.array([date.year, date.month, date.day, date.hour, date.minute, date.second], dtype='f8') for date in dates]
